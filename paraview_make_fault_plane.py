...
fault = np.loadtxt(
    ...
    delimiter=",",
    skiprows=1,
    dtype={"names": ("east", "north", "z"), "formats": (np.float, np.float, np.float)},
)
...
for f_arr in fault:
    flines.append(
        "{0:.0f} {1:.0f} {2:.2f}\n".format(
            (f_arr["north"] - north_0) / 1000.0,
            (f_arr["east"] - east_0) / 1000.0,
            -f_arr["z"] / 1000.0,
        )
    )
...
for jj in range(ny - 1):
    for ii in range(nx - 1):
        ...
        if idown:
            flines.append(
                "3 " + str(m) + " " + str(m + 1 + nx) + " " + str(m + nx) + "\n"
            )
            flines.append(
                "3 " + str(m) + " " + str(m + 1) + " " + str(m + 1 + nx) + "\n"
            )
        else:
