...
edi_path = Path(
    r"c:\Users\jpeacock\OneDrive - DOI\MountainPass\EasternMojave\modem_inv\skj"
)

save_path = Path(
    r"c:\Users\jpeacock\OneDrive - DOI\MountainPass\EasternMojave\modem_inv\inv_02"
)
...
# big map
# bounds = {"lat": np.array([32.5, 37.35]), "lon": np.array([-124.5, -114.15])}
bounds = None
# # SKJ line
# bounds = {"lat": np.array([35.95, 36.35]), "lon": np.array([-118.5, -116.15])}
...
dfn = save_path.joinpath("{0}_modem_data_z05_t02_add.dat".format(fn_stem))
...
if not dfn.exists():
    ...
    data_objmt_dict = data_obj.make_mt_dict()
...
if topography:
    ...
    mod_obj.add_topography_to_model2(topo_fn, airlayer_type="log_down", shift_north=0)
    mod_obj.write_model_file(
        model_fn_basename=r"{0}_modem_sm02_ocean.rho".format(fn_stem)
    )
