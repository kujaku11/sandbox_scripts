...
#        preplst.append((abs(dp1[0]-dp2[0])/dp1[0])*100) 
        ...
        ptd1=np.array([np.sqrt(np.linalg.det(pt1))*(180/np.pi)
                        for pt1 in ptdict1.pt])
        ptd2=np.array([np.sqrt(np.linalg.det(pt2))*(180/np.pi)
                        for pt2 in ptdict2.pt])
        ptlst.append((abs(ptd1-ptd2)/ptd1)*100)
        
        ptdet = (abs(ptd1-ptd2)/ptd1)*100
        
        for freq, zz, pp in zip(1./period, zdet, ptdet):
            lines.append('{0:<12}{1:>12.2f}{2:>12.2f}\n'.format(freq, zz, pp))
            
        rfid = file(os.path.join(sv_path, '{0}_repeat.txt'.format(station)), 
                    'w')
