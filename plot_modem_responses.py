...
class PlotResponse:
    ...
    def _read_files(self) -> None:
        ...
        for response_file in response_files:
            if response_file is None:
                continue
            response = MTData()
            response.from_modem(response_file, survey="model")
            self.resp_object.append(response)

    def _station_names(self, mt_data: MTData) -> list[str]:
        """Return station names in the order stored by an MTData container."""
        return [station_path.rsplit("/", 1)[-1] for station_path in mt_data._iter_station_paths()]

    def _get_mt_station(self, mt_data: MTData, station: str) -> MT:
        ...
        if len(matching_paths) != 1:
            raise KeyError(f"Expected one MTData station named {station!r}")
        return mt_data.get_station(matching_paths[0], as_mt=True)
    ...
    def _plot(self) -> None:
        ...
        ns = len(self._station_names(self.data_object))
        ...
        if self.plot_type != "1":
            ...
            for ii, station in enumerate(self._station_names(self.data_object)):
            ...
            pstation_list = self._station_names(self.data_object)

        for jj, station in enumerate(pstation_list):
            mt_obj = self._get_mt_station(self.data_object, station)
            z_obj = mt_obj.Z
            t_obj = mt_obj.Tipper
            period = z_obj.period
            ...
            if self.resp_object is not None:
                for resp_obj in self.resp_object:
                    response_mt = self._get_mt_station(resp_obj, station)
        ...
        for jj, station in enumerate(pstation_list):
            ...
            if plotr == True:
                for rr in range(nr):
                    ...
                    response_mt = self._get_mt_station(self.resp_object[rr], station)
                    resp_z_obj = response_mt.Z
                    resp_z_err = np.nan_to_num((z_obj.z - resp_z_obj.z) / z_obj.z_err)
    ...
    def _plot_1col(self) -> None:
        ...
        if self.period_limits is None:
            self.period_limits = (
                10 ** (np.floor(np.log10(self._get_mt_station(self.data_object, self._station_names(self.data_object)[0]).Z.period[0]))) * 1.01,
                10 ** (np.ceil(np.log10(self._get_mt_station(self.data_object, self._station_names(self.data_object)[0]).Z.period[-1]))) * 0.99,
            )
    ...
    def save_figure(
        ...
        close_fig : {"y", "n"}, optional
            Whether to close the active figure after saving, by default ``"y"``.
        """
