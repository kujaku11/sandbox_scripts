...
class XMLMetadata(object):
    ...
    def _set_id_info(self):
        ...
        for name in ["westbc", "eastbc", "northbc", "southbc"]:
            ET.SubElement(bounding, name).text = "{0:.1f}".format(
    ...
    def _set_data_quality(self):
        ...
        for step in range(1, step_num + 1, 1):
            ...
            ET.SubElement(processing_step_01, "procdesc").text = getattr(
                self.processing, "step_{0:02}".format(step)
            )
            ET.SubElement(processing_step_01, "procdate").text = getattr(
                self.processing, "date_{0:02}".format(step)
            )
    ...
    def _set_extra_info(self, station=False):
        ...
        if station is False:
            ...
            ET.SubElement(elev_rdom, "dommin").text = "{0:.0f}".format(
                self.survey.elev_min
            )
            ET.SubElement(elev_rdom, "dommax").text = "{0:.0f}".format(
    ...
    def _set_meta_info(self):
        ...
        ET.SubElement(meta_info, "metastdn").text = (
