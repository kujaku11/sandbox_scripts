...
# =============================================================================
v_fn_list = [
    os.path.join(v_dir, fn) for fn in os.listdir(v_dir) if fn.endswith(".depth")
]
...
# get shifts
mt_east, mt_north, mt_zone = gis_tools.project_point_ll2utm(mt_center[0], mt_center[1])
v_east, v_north, v_zone = gis_tools.project_point_ll2utm(
    center_point[0], center_point[1]
)

shift_east = (mt_east - v_east) / 1000.0
shift_north = (mt_north - v_north) / 1000.0
...
gridToVTK(
    os.path.join(v_dir, "roland_vp"), vtk_y, vtk_x, vtk_z, cellData={"Vp": vp_arr}
)

gridToVTK(
    os.path.join(v_dir, "roland_vs"), vtk_y, vtk_x, vtk_z, cellData={"Vs": vs_arr}
)

gridToVTK(
    os.path.join(v_dir, "roland_vpvs"), vtk_y, vtk_x, vtk_z, cellData={"VpVs": vpvs_arr}
)
