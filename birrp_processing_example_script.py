...
### make a time array
t = np.arange(n_samples) * 1.0 / sampling_rate
...
for comp in ["ex", "ey", "hx", "hy", "hz"]:
    ...
    ts_obj.ts = np.sum(
        [
            amp * np.sin(2 * np.pi * omega * t + phi)
            for amp, omega, phi in zip(
                np.random.random(50), np.logspace(-3, 3, num=50), np.random.random(50)
            )
        ],
        axis=0,
    )
...
for ii, comp in enumerate(["ex", "ey", "hx", "hy", "hz"]):
    ...
    script_obj.fn_arr[0][ii][
        "nskip"
    ] = 22  ### birrp only reads in numbers so need to skip the header
...
j2edi.birrp_config_fn = (
    r"/path/to/birrp/configuration/file"  ### here it would be example_birrp_config.cfg
)
j2edi.birrp_dir = (
    r"/path/to/station/birrp/folder"  ### here it would be the current directory
)
