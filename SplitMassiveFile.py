...
rr_fn2 = r"c:\MT\iMush\geh017_20140814_000500_1024_HX.Z3D"
save_path = os.path.dirname(rr_fn2)
comp = "HX"

starttime = "2014-08-10,00:05:00"
dt_fmt = "%Y-%m-%d,%H:%M:%S"
...
stamp_lst = ["gps", "time", "lat", "lon", "status", "gps_accuracy", "temperature"]
...
data_type = np.dtype([(st, dt) for st, dt in zip(stamp_lst, data_types)])

block3_seconds = 5 * 60  # 4096 for 5 minutes
block1_seconds = 15 * 60  # 1024 for 15 minutes
block2_seconds = 11 * 60 * 60 + 40 * 60  # 256 for 5 hours and 40 minutes
...
t3 = float(t1)
for ii in range(15):
    for blen, bsec, bdf, bdec, nn in zip(
        block_lst, block_seconds, block_df, block_dec, range(len(block_lst))
    ):
        dt2 = time.localtime(t2)
...
while ss < 45:
    while tt < 3:
        print "=" * 60
        print "Start {2} Schedule={0},  df={1}".format(
            sdt_lst[ss], block_df[tt], time.ctime()
        )
        # change information in header block to fit the new file
        new_df = block_df[tt]
        new_header = header2.replace("2014-08-14,00:05:00", sdth_lst[ss])
        new_header = new_header.replace("1024", str(new_df))
        ...
        while gt != sdt_lst[ss] and ss <= block3_seconds and jj <= block_seconds[tt]:
            if ii == 0:
            ...
            if gps_index != -1:
                ...
                stamp_array[jj], gps_index, gps_dweek = zt.get_gps_stamp(gps_index)
                if stamp_array[jj]["time"] < 86400 * 2:
                    # if gps_dweek != 0:
                    gt = zt.get_date_time(gps_week, stamp_array[jj]["time"])
                    new_header = new_header.replace(str(gps_week), str(gps_week + 1))
                else:
                    gt = zt.get_date_time(gps_week, stamp_array[jj]["time"])
                logfid.write("{0}{1}  {2}  {3}".format(" " * 4, jj, gt, rfid2.tell()))
                if np.remainder(jj, 4) == 0:
                    logfid.write("\n")
                jj += 1

                # read in time series before the gps stamp
                ts_int_array = np.fromstring(zt._raw_data[0:gps_index], dtype=np.int32)
                ...
                if (
                    len(zt._raw_data[gps_index + gps_stamp_len :]) + ii
                    < ts_array.shape[0]
                ):
                    ts_int_array = np.fromstring(
                        zt._raw_data[gps_index + gps_stamp_len :], dtype=np.int32
                    )
                    nts = ts_int_array.shape[0]
                    ts_array[ii : ii + nts] = ts_int_array
                    ii += nts
        ...
        # ----- write to a file ----
        new_date = sdt_lst[ss][0:10].replace("-", "")
        new_time = sdt_lst[ss][11:20].replace(":", "")
        new_fn = "geh017_{0}_{1}_{2}_{3}.Z3D".format(
            new_date, new_time, block_df[tt], comp.upper()
        )
        ...
        if dec != 1:
            ...
            for nn in range(stamp_array.shape[0]):
                ...
                ts_blockf, filt_lst = mtfilt.adaptive_notch_filter(ts_block, **nfkwargs)

                new_ts_block = sps.resample(ts_blockf, int(df0 / dec), window="hanning")
        ...
        new_file = os.path.join(r"c:\MT\iMush\geh017", new_fn)
        nfid = file(new_file, "wb")
        nfid.write(new_header)
        for ll in range(stamp_array.shape[0]):
            # write gps stamp
            for skey, stype in zip(stamp_lst, ["i", "i", "d", "d", "I", "i", "f"]):
                ...
                # make sure lat and lon are in radians
                    ...
                    nfid.write(struct.pack(stype, stamp_array[ll][skey] * np.pi / 180))
            ...
            nfid.write(
                struct.pack("i" * new_df, *ts_array[ll * new_df : (ll + 1) * new_df])
            )

        nfid.close()
        print "wrote file to {0}".format(new_file)
        print "Ended at {0}".format(time.ctime())
        logfid.write("-" * 72 + "\n")
        logfid.write("{0}{1}".format(" " * 4, new_file))
        logfid.write("-" * 72 + "\n")
        # reset values
        ss += 1
        tt += 1
