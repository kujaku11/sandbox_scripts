...
depth = np.zeros((m_obj.nodes_north[pad:-pad].shape[0],
                  m_obj.nodes_east[pad:-pad].shape[0]))
...
depth[np.where(depth/1000. > d_max)] = np.nan
im = ax.pcolormesh(x/1000., y/1000., depth/1000., 
                   vmin=0, vmax=d_max, cmap=cmap)
q = ax.contour(x/1000., y/1000., depth/1000., 10, 
               vmin=.05, vmax=d_max, cmap=cmap)
cb = plt.colorbar(im, shrink=.75)
cb.set_label('Depth bmsl (km)', fontdict={'size':13, 'weight':'bold'})

ax.scatter(d_obj.station_locations.rel_east/1000.,
           d_obj.station_locations.rel_north/1000.,
           marker='v', s=20, c='k', zorder=30)
...
fig.savefig(r"c:\Users\jpeacock\OneDrive - DOI\Geysers\jvgr\gz_depth_{0:.0f}ohmm.pdf".format(rho),
            dpi=300)
