...
for ii, key in enumerate(["surface", "middle_crust", "lower_crust"]): 
    ...
    ax.scatter(d.station_locations.rel_east, d.station_locations.rel_north, marker='v', s=20)
    ax.set_xlim((m.grid_east[pad], m.grid_east[-pad]))
    ax.set_ylim((m.grid_north[pad], m.grid_north[-pad]))
    
    cb = plt.colorbar(im, ax=ax, shrink=.35)
    

    array2raster.array2raster(Path(m.save_path).joinpath(f"canv_conductance_impedance_{key}.tiff").as_posix(),
