...
#        print key+': '+str(prepdict[key])
    ...
    try:
        ...
        if type(fdictc) is str and fdictc!='-1':
...
def convertBIRRPoutputs(bfpath,stationinfofile,station,rrstation=None,
    ...
    try:
        impfile=writeimp(os.path.join(bfpath,station+'.j'),egain=egain,
                         dlgain=dlgain,dlen=dlen,magtype=magtype,
                         bbfile=bbfile,tsp='   ',ffactor=ffactor)
    except IndexError:
...
#            stop=combstr[tspot+2:tspot+4]+'0000'
    ...
    if save=='y':
        ...
        print '\t '+os.path.join(plotpath,station+day+st+'Resxy.pdf')
        print '\t '+os.path.join(plotpath,station+day+st+'Resxx.pdf')
        if cfilelst!=None:
...
def readj(jfn,egain=1,dlgain=1,dlen=[50,50],magtype='bb',
    ...
    try:
        ...
        raise ValueError('Could not read .j file')
    z=np.zeros((4,3,nf))
