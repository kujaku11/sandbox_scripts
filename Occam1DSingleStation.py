...
if plot == "y":
    try:
        resp_tm_list = [
            os.path.join(save_dir, station, "TM_{0}.resp".format(rr))
            for rr in range(1, iter_num + 1)
        ]
        iter_tm_list = [
            os.path.join(save_dir, station, "TM_{0}.iter".format(rr))
            for rr in range(1, iter_num + 1)
        ]
        resp_te_list = [
            os.path.join(save_dir, station, "TE_{0}.resp".format(rr))
            for rr in range(1, iter_num + 1)
        ]
        iter_te_list = [
            os.path.join(save_dir, station, "TE_{0}.iter".format(rr))
            for rr in range(1, iter_num + 1)
        ]
        model_fn = os.path.join(ocd.save_path, "Model1D")

        p1 = occam1d.Plot1DResponse(
            data_tm_fn=data_tm_fn,
