...
data_obj.write_data_file(
    save_path=save_path,
    fn_basename="torc_modem_data_z{0:02.0f}_t{1:02.0f}.dat".format(
        data_obj.error_value_z, 100 * data_obj.error_value_tipper
    ),
)
...
mod_obj.write_model_file(
    model_fn_basename=os.path.join(save_path, r"torc_modem_sm_02.rho")
)
...
mod_obj.write_vtk_file(vtk_save_path=save_path, vtk_fn_basename="torc_sm_topo")
data_obj.write_vtk_station_file(
    vtk_save_path=save_path, vtk_fn_basename="torc_stations_topo"
)
