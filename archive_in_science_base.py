...
for station in [os.listdir(survey_dir)[5]]:
    ...
    if os.path.isdir(station_path):
        ...
        asc_fn_list = ["{0}{1}".format(stem + station, ext) for ext in [".edi", ".png"]]
        ...
        with Capturing() as output:
            for fn_block in fn_list:
                ...
                for key in zm.channel_dict.keys():
                    zm.channel_dict[key]["InstrumentID"] = (
                        "ZEN" + zm.channel_dict[key]["InstrumentID"]
                    )
                zm.write_asc_file(save_dir=station_save_dir, full=False, compress=True)
                asc_fn_list.append(
                    os.path.basename(
                        zm._make_file_name(save_path=station_save_dir, compression=True)
                    )
                )
                zm.write_station_info_metadata(
                    save_dir=station_save_dir, mtft_bool=mtft_find
                )
            ...
            s_xml.write_xml_file(
                os.path.join(station_save_dir, "{0}_meta.xml".format(stem + station))
            )

        # --> write log file
        log_fid = open(
            os.path.join(station_save_dir, "{0}_Archive.log".format(stem + station)),
            "w",
        )
        log_fid.write("\n".join(output))
        log_fid.close()
        ...
        print(
            "--> Archived station {0}, took {1} seconds".format(
                station, station_diff.total_seconds()
            )
        )
...
survey_xml.write_xml_file(os.path.join(save_dir, "{0}.xml".format(stem + survey)))
