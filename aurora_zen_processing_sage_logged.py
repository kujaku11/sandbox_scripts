...
survey_dir = Path(r"c:\Users\jpeacock\OneDrive - DOI\MTData\SAGE2026\mth5")

# path to store EDI files and make directory if not already exists
edi_path = survey_dir.joinpath("EDI_Files_aurora_1")
...
# geomagnetic H5 file
geomag_mth5 = Path(
    r"c:\Users\jpeacock\OneDrive - DOI\MTData\SAGE2026\mth5\usgs_geomag_bou_xy.h5"
)
...
station_list = [
    # {"local": "sg2604", "remote": "sg2608"},
    {"local": "sg2608", "remote": "sg2604"},
    {"local": "sg2606", "remote": "sg2607"},
    # {"local": "sg2607", "remote": "sg2606"},
    # {"local": "sg2609", "remote": "sg2614"},
    # {"local": "sg2614", "remote": "sg2609"},
    # {"local": "sg2602", "remote": "sg2613"},
    {"local": "sg2613", "remote": "sg2602"},
]
    ...
    with logger.contextualize(station=station_id):
        ...
        try:
            ...
            for sample_rate in sample_rates:
                ...
                # # pick runs to use for processing.  This is a list of run numbers to use for processing.
                # if sample_rate in [4096, 256]:
                #     print(run_summary.mini_summary)
                #     cutoff_time = pd.to_datetime("2026-06-22T13:00:00Z", utc=True)
                #     run_summary.df = run_summary.df.loc[run_summary.df.end <= cutoff_time]
                #     print(run_summary.mini_summary)
                # elif sample_rate == 1:
                #     run_summary.df["end"] = pd.to_datetime("2026-06-22T13:00:00Z", utc=True)
                #     print(run_summary.mini_summary)
            ...
            for sample_rate, processed in processed_dict.items():
                if "tf" not in processed:
                    ...
                    continue
                md.add_station(processed["tf"])

            p2 = md.plot_mt_response(
                md.station_paths[::-1], plot_style="compare", fig_num=2
            )
            p2.save_plot(
