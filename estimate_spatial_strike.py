...
for k, m in d_obj.mt_dict.items():
    for st, st_err, sk, sk_err, per, tip in zip(m.pt.azimuth, m.pt.azimuth_err,
                                           m.pt.skew, m.pt.skew_err, m.pt.freq,
                                           m.Tipper.angle_real):
        tip = tip + 90
        if tip > 90:
            tip -= 180
        line = {'station': m.station,
                'latitude': m.latitude,
...
df.to_csv(r"c:\Users\jpeacock\OneDrive - DOI\Geothermal\GabbsValley\gv_mt_strike_and_skew.csv",
          index=False)
