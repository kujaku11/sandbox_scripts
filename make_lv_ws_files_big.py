...
inv_periods = np.array([.01, .1, .3, 1, 3, 10, 30, 100])

if not os.path.isdir(save_path):
    os.mkdir(save_path)

station_list = ['mb{0:03}'.format(ii) for ii in [178, 177, 220, 176, 222, 221, 
                175, 225, 188, 235, 236, 237, 190, 226, 179, 180, 181, 
                231, 189, 237, 232, 191, 192, 182, 197, 198, 196, 199, 200,
                300, 301, 302, 303, 315, 316, 317, 318, 306, 307, 308, 
                309, 311, 339, 320, 305, 323, 326]]
pw_station_list = ['lv{0:02}'.format(ii) for ii in [22, 21, 20, 14, 16, 
                   17, 18, 19, 12, 2, 13, 10, 9]] 
...
ws_mesh.cell_size_east = 500.
ws_mesh.cell_size_north = 500. 
ws_mesh.n_layers = 35
ws_mesh.pad_z = 3
ws_mesh.pad_east = 8
ws_mesh.pad_north = 8
...
ws_mesh.make_mesh()
ws_mesh.plot_mesh()
ws_mesh.write_initial_file(initial_fn=os.path.join(save_path, 'WSInitialMesh500m_shallow'),
                           res_list=[30]) 
...
ws_data.write_data_file(save_path=save_path,
                        data_basename='WS_data_{0}_8_shallow.dat'.format(folder))
