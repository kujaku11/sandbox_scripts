...
for folder in os.listdir(station_dir):
    ...
    if os.path.isdir(os.path.join(station_dir, folder)) is True:
        ...
        z3d_fn = [os.path.join(station_dir, folder, fn) for fn in 
                  os.listdir(os.path.join(station_dir, folder)) 
                  if fn[-4:] == '.z3d'][0]
...
zen_processing.mtcfg.write_dict_to_configfile(config_dict, 
                                  os.path.join(station_dir, 
                                               'StationInfo{0}.cfg'.format(f_stem)))