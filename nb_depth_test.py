...
for ii, mt_key in enumerate(sorted(d_obj.mt_dict.keys())):
    ...
    for jj, dd in enumerate(d_arr):
        log_fid.write('{0:<10.5g}{1:<10.2f}{2:<10.2f}\n'.format(dd['period'],
                      dd['depth_min'], dd['depth_max']))
    depth_arr_min[:, ii] = d_arr['depth_min']
    depth_arr_max[:, ii] = d_arr['depth_max']
    
log_fid.close()

depth_avg_min = np.array([depth_arr_min[kk, np.nonzero(depth_arr_min[kk, :])].mean()
                      for kk in range(d_obj.period_list.shape[0])])
depth_avg_max = np.array([depth_arr_max[kk, np.nonzero(depth_arr_max[kk, :])].mean()
                      for kk in range(d_obj.period_list.shape[0])])
