...
ll_utm = {
    ...
    "easting_min": 373200,
    "easting_max": 382500,
    "northing_min": 5053300.0,
    "northing_max": 5059700.0,
}

spacing_east = 1000.0
spacing_north = 1000.0


east_arr = np.arange(
    ll_utm["easting_min"], ll_utm["easting_max"] + spacing_east, spacing_east
)

north_arr = np.arange(
    ll_utm["northing_min"], ll_utm["northing_max"] + spacing_east, spacing_east
)
...
for ii, east in enumerate(east_arr):
    ...
    for jj, north in enumerate(north_arr):
        ...
        lat_ii, lon_jj = gis_tools.project_point_utm2ll(east, north, ll_utm["zone"])

        kml_obj.newpoint(name="um{0:03}".format(count), coords=[(lon_jj, lat_ii)])
