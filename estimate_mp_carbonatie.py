...
label_dict = {
    "phi_r": {"title": "Rock", "x_label": "Percent"},
    "phi_f": {"title": "Fluid", "x_label": "Percent"},
    "res": {"title": "Bulk Rock", "x_label": "Resistivity ($\Omega \cdot m$)"},
    "sigma_f": {
        "title": "Fluid Resistivity",
        "x_label": "Resistivity ($\Omega \cdot m$)",
    },
}

g = rp.Glover()
g.phase_01.from_dict({"sigma": 1.0 / 1e4, "phi": 0.88, "m": 0.05, "label": "rock"})
# g.phase_02.from_dict({'sigma':1./.5, 'phi':.08, 'm':3., 'label':'EDL'})
# g.phase_02.from_dict({'sigma':1./1, 'phi':.04, 'm':1.5, 'label':'fluids'})
g.phase_02.from_dict({"sigma": 1.0 / 10, "phi": 0.05, "m": 1.05, "label": "fluids"})

res_min, res_max = (100, 200)
n = int(1e6)
res_dict = {"res": [], "phi_r": [], "phi_f": [], "sigma_f": []}
for phi_r, sigma_f in zip(np.random.normal(0.85, 0.15, n), np.random.normal(2, 2, n)):
    ## np.random.lognormal(.1, .08, n)
    if phi_r > 1:
        continue
    g.phase_01.phi = phi_r
    g.phase_02.phi = 1 - phi_r
    g.phase_02.sigma = 1.0 / sigma_f
    ...
    if res >= res_min and res <= res_max:
        ...
        res_dict["sigma_f"].append(1.0 / g.phase_02.sigma)
...
for ii, col in enumerate(df.columns, 1):
    ...
    if col not in ["res"]:
        ...
        num, bins, patches = ax.hist(
            df[col],
            bins=np.logspace(np.log10(res_min), np.log10(res_max), 100),
            density=1,
        )
        pdf, pdf_min, pdf_max = rp.fit_distribution(bins, params, best_d)
        (f,) = ax.plot(bins, pdf)
    ...
    ax.grid(which="major", ls=":", lw=0.75, color=(0.6, 0.6, 0.6))
    ax.set_axisbelow(True)
    mlk = bins[np.where(pdf == pdf.max())][0]
    (lk,) = ax.plot([mlk, mlk], [0, num.max()], lw=3, ls=":", color="k")
    (l_max,) = ax.plot(
        [pdf_max, pdf_max], [0, num.max()], lw=3, ls=":", color=(0.5, 0.5, 0.5)
    )

    (l_min,) = ax.plot(
        [pdf_min, pdf_min], [0, num.max()], lw=3, ls=":", color=(0.5, 0.5, 0.5)
    )
    (l2,) = ax.plot([None, None], [None, None], "b", ls="none")
    mlk_data = bins[np.where(num == num.max())][0]
    ax.legend(
        [l2, l2, lk, l_min, l_max, l2],
        [
