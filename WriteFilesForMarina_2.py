...
station_list_ne = [
    "pb{0:02}c".format(ii) for ii in [53, 52, 35, 51, 12, 11, 1, 23, 24, 48, 49, 50]
]
station_list_nw = [
    "pb{0:02}c".format(ii) for ii in [56, 55, 35, 54, 12, 11, 1, 23, 24, 25, 45, 46, 47]
]
...
for slst in [station_list_ew, station_list_ns, station_list_ne, station_list_nw]:
    ...
    for ss, station in enumerate(station_list):
        ...
        if os.path.isfile(fn_base) == True and os.path.isfile(fn_inj) == True:
            ...
            sx = np.sqrt(
                np.mean(
                    edi_base.Z.resistivity[0:15, 0, 1]
                    / edi_inj.Z.resistivity[0:15, 0, 1]
                )
            )
            sy = np.sqrt(
                np.mean(
                    edi_base.Z.resistivity[0:15, 1, 0]
    ...
    for ss, station in enumerate(station_list):
        ...
        new_fn_base = os.path.join(
            sv_path_base, os.path.basename(fn_base)[:-4] + "f.edi"
        )
        new_fn_inj = os.path.join(sv_path_inj, os.path.basename(fn_inj)[:-4] + "f.edi")
