...
# Make the data file
# ==============================================================================
inv_period_list = np.logspace(np.log10(0.003130), np.log10(10922.6), num=23)
...
data_obj.data_array["tip"][np.where(np.abs(data_obj.data_array["tip"] < 0.001))] = (
    0.0 + 1j * 0.0
)
data_obj.data_array["tip"][np.where(np.abs(data_obj.data_array["tip"] > 1.3))] = (
    0.0 + 1j * 0.0
)
...
mod_obj.add_topography_to_model2(
    topographyfile=topo_fn, airlayer_type="log", max_elev=1250
)
...
mod_obj.write_model_file(
    model_fn_basename=os.path.join(
        save_path, r"{0}_modem_sm02_topo.rho".format(fn_stem)
    )
)
