...
rpt_arr = np.zeros(5, dtype=[('station', 'S10')])

repeat_list = ['station,repeatability(%),repeatability_err(%)']
for station in ([3, 4, 13, 24, 30]):
    ...
    ss_x = 1./np.sqrt(np.median(np.abs(mt_02.Z.z[no_change, 0, 1]/mt_01.Z.z[no_change, 0, 1])))
    ss_y = 1./np.sqrt(np.median(np.abs(mt_02.Z.z[no_change, 1, 0]/mt_01.Z.z[no_change, 1, 0])))
    ...
    print 'TNG-0{0:02}, Repeatability is {1:.2f}% +/- {2:.2f}%'.format(station,
                                                                repeat,
                                                                repeat_err)
    repeat_list.append('TNG-0{0:02},{1:.2f}%,{2:.2f}'.format(station,
                                                              repeat,
                                                              repeat_err))

with open(r"c:\Users\jrpeacock\Documents\Test_Data\Tongario\repeatability_pt_det.txt", 'w') as fid:
    fid.write('\n'.join(repeat_list))
