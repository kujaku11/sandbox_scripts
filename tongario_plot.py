...
fig.subplots_adjust(left=0.1, bottom=0.06, right=0.98, top=0.95, hspace=0.07)
line_list = []
label_list = []
plot_period = 1.0 / rpt_arr["frequency"][0]
interp_period = np.arange(plot_period.min(), plot_period.max())

noise = signal.medfilt(np.mean(rpt_arr["phimax"][noiseless], axis=0), kernel_size=7)
...
for ii, r_arr in enumerate(rpt_arr[plot_station_index]):
    ...
    ax.grid(which="both", color=(0.75, 0.75, 0.75), ls="--", alpha=0.7)
