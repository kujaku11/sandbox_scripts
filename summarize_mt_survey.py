...
df_dict = {256:datetime.timedelta(hours=7, minutes=45),
...
if not csv_path.exists():
    ...
    for station in survey_path.glob('**/*'):
        ...
        for z3d_fn in station_path.glob('*.Z3D'):
            z_obj = zen.Zen3D(Path.joinpath(station_path.parent, station_path.name,
                                         z3d_fn))
            try:
                z_obj.read_all_info()
...
for index, row in survey_02.iterrows():
    ...
    ax1.plot_date([row['start_date'], end_time],
                  [xdata, xdata],
                  color=color,
                  xdate=True,
                  ydate=False,
