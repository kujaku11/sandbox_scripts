...
edi_list = [
    fn
    for fn in glob.glob(
        r"c:\Users\jpeacock\OneDrive - DOI\med_report\data\saudi_edi_geographic\*.edi"
    )
...
for edi_fn, rot in zip(sorted(edi_list), [-120, 10, -120, 10, -30, -120]):
    ...
    p.axr.set_ylabel(
        "Apparent Resistivity ($\Omega$m)", fontdict={"size": 10, "weight": "normal"}
    )
    ...
    p.update_plot()
    p.save_plot(
        r"c:\Users\jpeacock\OneDrive - DOI\med_report\report\figures\figure_05_{0:02}.svg".format(
            count
        ),
        fig_dpi=300,
    )
