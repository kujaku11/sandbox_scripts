...
x = (np.array(sorted(list(set(gh_model[:, 0]))))-msh_center.easting)/1000.
...
pb_x_grid, pb_y_grid = np.meshgrid((m_obj.grid_east[0:-1]-east_diff)/1000., 
...
eq_arr = np.loadtxt(s_fn,
                    usecols=(0, 2, 3, 4, 5, 6),
                    dtype={'names': ('mag', 'lat', 'lon', 'depth', 'northing', 'easting'),
                           'formats': ('f8', 'f8', 'f8', 'f8', 'f8', 'f8')})
eq_arr = eq_arr[np.where((eq_arr['northing'] > msh_center.northing-1000) &
...
lp_arr = np.loadtxt(lp_fn,
                    delimiter=',',
                    skiprows=1,
                    usecols=(2, 3, 4, 5),
                    dtype={'names':('east', 'north', 'depth', 'mag'),
                           'formats':('f8', 'f8', 'f8', 'f8')})
lp_arr = lp_arr[np.where((lp_arr['north'] > msh_center.northing-5000) &
...
fig.subplots_adjust(hspace=.12, left=.01, right=.92, bottom=.12, top=.9)
...
for ax in [ax_gh, ax_pb]:
    ax.scatter((eq_arr['easting']-msh_center.easting)/1000.,
               eq_arr['depth']+2.25,
               marker='.',
               c='k',
               s=1)
    ax.scatter((lp_arr['east']-msh_center.easting)/1000.,
               lp_arr['depth']+2.25,
               marker='*',
               c='k',
               s=5)
    ...
    ax.tick_params(which='minor', width=.5)
