...
ws_station_fn = (
    r"/home/jpeacock/Documents/wsinv3d/LV/Inv_lv_coarse_2/WS_Station_Locations.txt"
)
...
north, east = np.broadcast_arrays(
    modem_mod.grid_north[:, None], modem_mod.grid_east[None, :]
)

# 2) do a 2D interpolation for each layer, much faster
new_res = np.zeros(
    (ws_mesh.grid_north.shape[0], ws_mesh.grid_east.shape[0], ws_mesh.grid_z.shape[0])
)
...
for zz in range(ws_mesh.grid_z.shape[0]):
    ...
    print "New depth={0:.2f}; old depth={1:.2f}".format(
        ws_mesh.grid_z[zz], modem_mod.grid_z[old_zz]
    )
...
ws_mesh.write_initial_file(
    initial_fn=r"/home/jpeacock/Documents/wsinv3d/LV/ws_sm_modem_lv",
    res_model=new_res,
    res_list=None,
)
...
ax2.pcolormesh(
    mx, my, np.log10(modem_mod.res_model[:, :, 27]), cmap="jet_r", vmin=-1, vmax=4
)
ax2.scatter(
