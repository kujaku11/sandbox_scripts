...
locations = np.zeros(ns, dtype=[('name', '|S10'), 
                                ('lat', np.float),
                                ('lon', np.float)])
loc_dict = {}
...
for jj, line in enumerate(data_fid.readlines()):
    ...
    if f_index == nf:
        data_list.append({'name':name, 'res':res, 'phase':phase, 
                          'res_err':res_err, 'phase_err':phase,
        ...
        data_z.set_res_phase(res, phase,
                             reserr_array=res_err, phaseerr_array=phase_err)
        ...
        #---------------HEADER----------------------------------------------
        data_edi.head = dict([('dataid', name),
        ...
        #----------------DEFINE MEASUREMENT----------------------------------
        data_edi.definemeas = dict([('maxchan', 5),
                                    ('maxrun', 999),
                                    ('maxmeas', 99999),
                                    ('units', 'm'),
                                    ('reftype', 'cartesian'),
        ...
        #------------MTSECT------------------------------------------------
        data_edi.mtsect = dict([('sectid', name),
        ...
        #--> write .edi file
        edi_fn = data_edi.writefile(os.path.join(save_path, 
                                                 '{0}.edi'.format(name)))
