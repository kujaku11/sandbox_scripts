...
#                                                                     model_center[1])
model_zone, model_east, model_north = gis_tools.ll_to_utm(23, 
                                                          model_center[0],
                                                          model_center[1])
...
pointsToVTK(r"c:\Users\jpeacock\OneDrive - DOI\MountainPass\paraview_files\mp_eq", 
            vtk_arr['north'].copy(),
            vtk_arr['east'].copy(),
            vtk_arr['depth'].copy(),
            data={'depth':vtk_arr['depth'].copy(),
                  'mag':vtk_arr['mag'].copy()})
...
geo_df = gpd.GeoDataFrame(df.drop(['lat', 'lon'], axis=1),
                          crs=datum,
