...
### locate drill hole 
e_index = np.where((m_obj.grid_east <= drill_hole['east']+40) &
                   (m_obj.grid_east >= drill_hole['east']-40))[0][0]
n_index = np.where((m_obj.grid_north <= drill_hole['north']+40) &
                   (m_obj.grid_north >= drill_hole['north']-40))[0][0]
...
ax.set_xlim((10, 500))
ax.set_ylim((1620, 0))
ax.grid(which='major', ls='--', color=(.65, .65,.65), lw=1.2)
ax.grid(which='minor', ls='-.', color=(.75, .75,.75), lw=1)
