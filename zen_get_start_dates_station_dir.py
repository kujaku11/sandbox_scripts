...
for station in os.listdir(survey_path):
    ...
    if os.path.isdir(station_path) is True:
        ...
        station_date_arr = np.zeros(
            len(fn_list), dtype=[("df", np.float), ("start_dt", "|S20")]
        )
...
for key in sorted(rr_dict.keys()):
    ...
    for k_tuple in rr_dict[key]:
        lines.append(
            "\tStation: {0}, Sampling Rate: {1:.0f}".format(k_tuple[0], k_tuple[1])
        )
...
if plot is True:
    ...
    df_dict = {4096: (0.7, 0.1, 0), 1024: (0.5, 0.5, 0), 256: (0, 0.2, 0.8)}
    ...
    for k_index, key in enumerate(sorted(date_dict.keys())):
        ...
        for ii, k_arr in enumerate(date_dict[key]):
            ...
            try:
                x_date_1 = datetime.datetime.strptime(
                    date_dict[key]["start_dt"][ii + 1], datetime_fmt
                )
            except IndexError:
                x_date_1 = x_date_0
            ...
            (l1,) = ax.plot(
                [x_date_0, x_date_1], y_values, lw=8, color=df_dict[k_arr["df"]]
            )
    ...
    ax.grid(which="major", linestyle="--", color=(0.7, 0.7, 0.7))
    ...
    fig.legend(
        [l_4096, l_1024, l_256], ["4096", "1024", "256"], ncol=3, loc="upper center"
    )
