...
for station in os.listdir(survey_dir):
    ...
    if os.path.isdir(station_path):
        ...
        zm = archive.USGSasc()
        asc_fn_list = [
            "{0}{1}".format(station.upper(), ext) for ext in [".edi", ".png"]
        ]
        ...
        # --> write log file
        log_fid = open(
            os.path.join(station_save_dir, "{0}_Archive.log".format(station)), "w"
        )
        log_fid.write("\n".join(output))
        log_fid.close()
        ...
        s_time = s_et - s_st
        print(
            "--> Archiving {0}. Took {1} seconds".format(
                station, s_time.total_seconds()
            )
        )
        print("-" * 40)
# =============================================================================
...
survey_xml.write_xml_file(
    os.path.join(save_dir, "{0}.xml".format(survey.replace(" ", "_")))
)
