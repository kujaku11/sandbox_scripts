...
ant_cal_fn = r"/mnt/hgfs/MT_Data/antenna.cal"
...
for line in lines:
    ...
    if line.find('antenna') > 0:
        ...
        if ant != 0:
            z_real = amp * np.cos(phase/1000.)
            z_imag = amp * np.sin(phase/1000.)
            
            z_real2 = amp * np.cos(phase/1000.) #
            z_imag2 = amp * np.sin(phase/1000.) #
            entry = [freq, amp, phase, amp2, phase2,
                     z_real, z_imag, z_real2, z_imag2]
...
for key in list(data.keys()):
    ...
    df = pd.DataFrame(data[key],
                      columns=['freq','amp','phase','amp2','phase2',
                               'z_real', 'z_imag', 'z_real2', 'z_imag2'])
    df.to_csv(sv_fn, columns=['freq', 'z_real', 'z_imag'],
