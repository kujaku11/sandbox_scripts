...
v_lat = origin_lines[0]+origin_lines[1]/60.
v_lon = -1*(origin_lines[2]+origin_lines[3]/60.)
...
s_dtype = [(label, d_type) for label, d_type in zip(station_header[0:7],
           ['|S5', np.float, np.float, np.float, np.float, np.float, 
            np.float, np.float])]
...
for ii, sline in enumerate(station_lines):
    ...
    station_arr[ii][station_header[1]] = s_list[1]+s_list[2]/60.
    station_arr[ii][station_header[2]] = s_list[3]+s_list[4]/60.
...
n_east, n_north, nz = [int(ii) for ii in [grid_line[5], grid_line[8], 
                                          grid_line[11]]]
...
for ii, line in enumerate(loc_lines):
    ...
    eq_arr[ii][loc_header[0]] = loc_list[0]+loc_list[1]/60.
    eq_arr[ii][loc_header[1]] = loc_list[2]+loc_list[3]/60.
...
v_zone, v_east, v_north = ll2utm.LLtoUTM(23, v_lat, v_lon-.29)    
    
shift_east = (mt_east-v_east+4000)/1000.
shift_north = (mt_north-v_north+1000)/1000.
...
a2vtk.gridToVTK(os.path.join(save_dir, 'lv_3d_models', 'foulger_2003_vp_rot_lvc'), 
                vtk_grid_north, 
                vtk_grid_east, 
                vtk_gz, 
                cellData={'Vp': vtk_vp.T,
                          'd_Vp': vtk_d_vp.T,
                          'Vp/Vs': vtk_vp_vs.T,
                          'Vs': vtk_vs.T})
