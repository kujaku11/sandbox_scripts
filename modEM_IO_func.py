...
def read_modem_models(model_file, ref_loc):
    ...
    x0 = ref_loc - np.array([np.sum(hx) / 2.0, np.sum(hy) / 2.0, np.sum(hz)])
    ...
    mod_modem_convention = np.concatenate(
        [np.array(line.split(), float) for line in file_lines[4 : 4 + nr_mod_lines]]
    )
    mod_mat = mod_modem_convention.reshape(int(nr_y), int(nr_x), int(nr_z), order="F")[
        ::-1, :, ::-1
    ]
