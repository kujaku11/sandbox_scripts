...
#ts, dc_trend = remove_dc_trend(mat_dict[sorted(mat_dict.keys())[0]][:, 0])
ts, dc_trend = remove_dc_trend(ts_rand)
dt = 1.
nt = ts.shape[0]
t = np.arange(nt)*dt
...
# add in coil response
output_coil = add_coil_response(ts, dt)
...
fft_ts = np.fft.fft(ts)[0:nt]
fft_output_coil = np.fft.fft(output_coil)[0:nt]
fft_hp = np.fft.fft(hp_filter_output)[0:nt]
...
ax1p.semilogx(plot_freq, np.rad2deg(np.unwrap(np.arctan2(fft_ts.imag, 
                                               fft_ts.real))), 'k')

ax2p = fig.add_subplot(4, 3, 6, sharex=ax1f)
ax2p.semilogx(plot_freq, np.rad2deg(np.unwrap(np.arctan2(fft_output_coil.imag, 
                                               fft_output_coil.real))),
            'b')

ax3p = fig.add_subplot(4, 3, 9, sharex=ax1f)
ax3p.semilogx(plot_freq, np.rad2deg(np.unwrap(np.arctan2(fft_hp.imag,
                                                         fft_hp.real))), 'g')

ax4 = fig.add_subplot(4, 1, 4, sharex=ax1)
l1, = ax4.plot(t, ts, 'k')
l2, = ax4.plot(t, output_coil, 'b')
