...
while ss < 45:
    while tt < 3:
        ...
        new_fn = "mb156_{0}_{1}_{2}_{3}.Z3D".format(
        ...
        nfid.close()
        print "wrote file to {0}".format(os.path.join(r"c:\MT\mb1587", new_fn))
        print "Ended at {0}".format(time.ctime())
        logfid.write("-" * 72 + "\n")
        logfid.write("{0}{1}".format(" " * 4, os.path.join(r"c:\MT\mb187", new_fn)))
        logfid.write("-" * 72 + "\n")
        # reset values
        ss += 1
        tt += 1
