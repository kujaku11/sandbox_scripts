...
fn_path = Path(r"c:\Users\jpeacock\Documents\imush\G016-5")
sampling_rate = 4
t_buffer = 8 * 3600
...
def get_z3d_info(z3d_path):
    ...
    fn_list = [
        fn_path for fn_path in z3d_path.rglob("*") if fn_path.suffix in [".z3d", ".Z3D"]
    ]
    ...
    for fn in fn_list:
        ...
        fn_dict[z_obj.component].append(
            {"start": z_obj.zen_schedule.isoformat(), "df": z_obj.df, "fn": z_obj.fn}
        )
...
def combine_z3d_files(z3d_path, new_sampling_rate=4, t_buffer=8 * 3600):
    ...
    for comp in ["ex", "ey", "hx", "hy", "hz"]:
        ...
        ascii_fn = "{0}_combined_{1}.{2}".format(
            new_ts.station, int(new_ts.sampling_rate), new_ts.component
        )
