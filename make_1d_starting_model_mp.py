...
if os.path.isfile(pickle_fn) is False:
    ...
    for ii, d_arr in enumerate(d_obj.data_array):
        ...
        if os.path.isfile(os.path.join(s_dir, 'Occam1d_DataFile_DET.dat')) is False:
            ...
            remove_files = glob.glob('{0}\*.iter'.format(s_dir)) + \
                           glob.glob('{0}\*.resp'.format(s_dir)) 
        ...
        try:
            ...
            if os.path.join(sv_dir, '{0}.png'.format(d_arr['station'])) is False:
                pr = occam1d.Plot1DResponse(data_te_fn=os.path.join(s_dir, 
                                        ...
                                        depth_limits=(-1, 30))
                pr.save_figure(os.path.join(sv_dir,
                                            '{0}.png'.format(d_arr['station'])),
                               fig_dpi=600)
        except:
...
m_obj.write_model_file(save_path=r"c:\Users\jpeacock\Documents\MountainPass\modem_inv\inv_08",
