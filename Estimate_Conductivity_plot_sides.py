...
def basalt_ni(temperature, wtp_h2o):
    ...
    sigma_b = np.exp(2.172-(860.82-204.46*np.sqrt(wtp_h2o))/(temperature-1146.8))
...
T_b = 1400.+273.5                          # dacite temperature
...
#    line_color = (0, 0, wtp_h2o/(wtp_h2o_max))
    ...
    line_rhyolite, = ax.semilogx(1./sigma_ma_low, melt_percent,
                       '-',
                       lw=.5,
                       marker='s',
                       ms=marker_size,
    ...
    line_dacite, = ax.semilogx(1./sigma_ma_low, melt_percent,
                       '-.',
                       lw=.5,
                       marker='o',
                       ms=marker_size,
    ...
    line_basalt, = ax.semilogx(1./sigma_ma_low, melt_percent,
                       '-.',
                       lw=.5,
                       marker='*',
                       ms=marker_size,
...
ax.fill_between([5, 15], [0, 0], [.5, .5], 
                color=(.75, .75, .75), 
                alpha=.35)
                
ax.set_ylabel('Melt Percent', 
               fontdict=label_font_dict)
ax.set_xlabel('Bulk Resistivity ($\Omega \cdot$m)', 
               fontdict=label_font_dict)
ax.set_ylim(0, .5)
...
fig.savefig(r"c:\Users\jpeacock-pr\Documents\TexDocs\Figures\melt_percent_plot.pdf", 
            dpi=600) 
##==============================================================================
...
for host_rock in [50, 100, 500, 1000, 10000]:    
    #--> plot resistivity vs. temperature
    line_color = tuple(3*[1-.9*np.log10(host_rock)/(4.)])
#    line_color = (0, 0, wtp_h2o/(wtp_h2o_max))
    sigma_a = arrhenius(T, 2, pressure=.5)
    ...
    sigma_ma_low = modified_archies(1./host_rock, sigma_melt, 1-melt_percent)
    sigma_ma_low[0] = 1./host_rock
...
fig.savefig(r"c:\Users\jpeacock-pr\Documents\TexDocs\Figures\melt_percent_plot_02.pdf", 
            dpi=600) 