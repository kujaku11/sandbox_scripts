...
edi_path = (
    r"c:\Users\jpeacock\Documents\iMush\iMush_edited_edi_files_JRP\Rotated_m16_deg"
)
...
for edi in os.listdir(edi_path):
    ...
    if station in s_list:
        ...
        mt_obj.write_edi_file(
            new_fn=os.path.join(sv_path, "{0}_tip_rot.edi".format(station))
        )
        print "rotated tipper for {0}".format(edi)
        print "-" * 40
    else:
        shutil.copy(os.path.join(edi_path, edi), os.path.join(sv_path, edi))
