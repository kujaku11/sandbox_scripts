...
# User Inputs:
input_path = (
    r"c:\Users\jpeacock\OneDrive - DOI\Geysers\depth_slices"  # Folder with input data
)
...
pdCellSize = int(
    round(
        float(str(arcpy.GetRasterProperties_management(studyAreaRaster, "CELLSIZEX")))
    )
)
...
for rastLayer in raster_list[:]:
    ...
    arcpy.gp.ExtractValuesToPoints_sa(
        studyAreaPoints, evidenceRaster, StudyAreaPointsTemp, "NONE", "VALUE_ONLY"
    )
    arcpy.AddField_management(
        StudyAreaPointsTemp,
        os.path.splitext(rastLayer)[0],
        "DOUBLE",
        "",
        "",
        "",
    ...
    arcpy.CopyFeatures_management(
        StudyAreaPointsTemp, studyAreaPoints, "", "0", "0", "0"
    )
