...
edi_list = [
    os.path.join(edi_path, ss) for ss in os.listdir(edi_path) if ss.find(".edi") > 0
]
...
modem_center = (544750.0, 4220700.0)
pad = 3
cell_size = 50.0
res_air = 1e12
elev_cell = 2
...
e_east, e_north, elevation = modem.read_dem_ascii(
    dem_fn, cell_size=50, model_center=modem_center, rot_90=0
)

### 2.) interpolate the elevation model onto the model grid
m_elev = modem.interpolate_elevation(
    e_east, e_north, elevation, m_obj.grid_east, m_obj.grid_north, pad=1
)

m_elev[np.where(m_elev > 10)] = 10.0
### 3.) make a resistivity model that incoorporates topography
mod_elev, elev_nodes_z = modem.make_elevation_model(
...
m_obj.write_model_file(save_path=sv_path, model_fn_basename="sm_hydro_topography.rho")
