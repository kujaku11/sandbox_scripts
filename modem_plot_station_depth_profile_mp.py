...
# =============================================================================
d_fn = (
    r"c:\Users\jpeacock\Documents\MountainPass\modem_inv\inv_08\mp_modem_data_z05.dat"
)
m_fn = r"c:\Users\jpeacock\Documents\MountainPass\modem_inv\inv_08\mp_z05_c05_176.rho"

occam_path = r"c:\Users\jpeacock\Documents\MountainPass\EDI_Files_birrp"
...
for s_arr in d_obj.station_locations.station_locations:
    ...
    d_index = np.where(res_depth < 1e11)[0][0]

    # get occam 1D
    occam_iter_fn = os.path.join(occam_path, s_arr["station"].upper(), "Det_6.iter")
    occam_model_fn = os.path.join(occam_path, s_arr["station"].upper(), "Model1D")

    occam1d_model = occam1d.Model()
    occam1d_model.read_iter_file(iter_fn=occam_iter_fn, model_fn=occam_model_fn)
    ...
    lines.append(
        "\n".join(
            [
                "{0:<12.2f} {1:<12.3f}".format(d, res)
                for d, res in zip(m_obj.grid_z[0:30], res_depth[0:30])
            ]
        )
    )
    lines.append("-" * 40)
    ...
    lines.append(
        "\n".join(
            [
                "{0:<12.2f} {1:<12.3f}".format(d, res)
                for d, res in zip(depth_1d[0:46], res_1d[0:46])
            ]
        )
    )
    txt_fn = r"c:\Users\jpeacock\Documents\MountainPass\depth_profiles\{0}.txt".format(
        s_arr["station"]
    )
    ...
    l1 = ax.plot(res_depth, m_obj.grid_z[0:-1], ls="steps", color=(0, 0.45, 1))
    l2 = ax.plot(
        res_1d, depth_1d + m_obj.grid_z[d_index], ls="steps", color=(1, 0.6, 0.1)
    )

    ax.set_xscale("log")
    ax.set_ylim(2000, m_obj.grid_z[0])
    ...
    ax.grid(which="major", linestyle="--", color=(0.5, 0.5, 0.5))
    ax.grid(which="minor", linestyle=":", color=(0.75, 0.75, 0.75))
