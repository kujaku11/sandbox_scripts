...
survey_csv_fn = (
    r"c:\Users\jpeacock\OneDrive - DOI\MTData\BV2023\bv_channel_summary.csv"
)
survey_df = pd.read_csv(survey_csv_fn)
survey_df.start = pd.to_datetime(survey_df.start)
survey_df.end = pd.to_datetime(survey_df.end)

edi_path = Path(
    r"c:\Users\jpeacock\OneDrive - DOI\Geothermal\BuffaloValley\EDI_Files_aurora\edited\GeographicNorth"
)
save_path = edi_path.joinpath("updated")
...
for edi_fn in edi_path.glob("*.edi"):
    ...
    m = MT(fn=edi_fn)
    m.read()
    for run_id in m.station_metadata.run_list[1:]:
        m.station_metadata.remove_run(run_id)
    ...
    m.survey_metadata.citation_dataset.authors = "J. R. Peacock and GUMP team"
    m.survey_metadata.citation_dataset.doi = ""
    m.survey_metadata.citation_dataset.year = "2023"
    m.survey_metadata.comments = None
    m.survey_metadata.country = "USA"
    m.survey_metadata.datum = "WGS84"
    m.survey_metadata.geographic_name = "Buffalo Valley, NV"
    m.survey_metadata.name = "Buffalo Valley"
    ...
    m.survey_metadata.project = "INGENIOUS"
    ...
    m.survey_metadata.release_license = "CC-BY-1.0"
    ...
    m.survey_metadata.summary = "Focus area of INGENIOUS"
    m.survey_metadata.id = "BV2023"
    ...
    m.station_metadata.location.declination.value = -12.4
    ...
    m.station_metadata.provenance.software.name = "MTpy-v2"
    m.station_metadata.provenance.software.version = "2.0.5"
    ...
    m.station_metadata.transfer_function.processed_date = "2023-08-10"
    ...
    m.station_metadata.transfer_function.remote_references = [
        ...
        "Fresno Magnetic Observatory (1s data)",
    ]
