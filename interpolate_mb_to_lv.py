...
new_model_fn = (
    r"/home/jpeacock/Documents/ModEM/LV/sm_avg_inv1/lv_avg_err07_NLCG_018.rho"
)
...
if interpolation_dim == "2d":
    north, east = np.broadcast_arrays(
        old_mod.grid_north[:, None] + diff_north, old_mod.grid_east[None, :] - diff_east
    )
    ...
    north, east, vert = np.broadcast_arrays(
        old_mod.grid_north[:, None, None] + diff_north,
...
new_mod.write_model_file(
    save_path=r"/home/jpeacock/Documents/ModEM/LV", model_fn_basename="lv_mb_sm.rho"
)
...
ax1.scatter(
    data_obj.station_locations["rel_east"],
