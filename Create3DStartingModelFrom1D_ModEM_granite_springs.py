...
# Inputs
# ==============================================================================
model_fn = r"c:\Users\jpeacock\Documents\Geothermal\GraniteSprings\modem_inv\inv_01\gs_sm01.rho"
data_fn = r"c:\Users\jpeacock\Documents\Geothermal\GraniteSprings\modem_inv\inv_01\gs_modem_data_err03_tip02.dat"
...
if not os.path.exists(npy_fn):
    oned_res_arr = np.zeros(
        len(mdd.mt_dict.keys()),
        dtype=[
            ("station", "S10"),
            ("grid_east", np.float),
            ("grid_north", np.float),
        ...
        if os.path.exists(iter_fn):
            ...
            if plot:
                pr = occam1d.Plot1DResponse(
                    data_te_fn=os.path.join(occam_sv_path, "Occam1d_DataFile_DET.dat"),
                ...
                pr.save_figure(
                    os.path.join(save_dir, "{0}_1d_resp.png".format(mt_obj.station)),
                    fig_dpi=900,
                )
        else:
            ...
            try:
                ...
                if plot:
                    ...
                    pr.save_figure(
                        os.path.join(
                            save_dir, "{0}_1d_resp.png".format(mt_obj.station)
                        ),
                        fig_dpi=900,
                    )
                ...
                print "-" * 50
                print "\t{0} did not run properly, check occam1d files".format(
                    mt_obj.station
                )
                print "-" * 50
    ...
    np.save(
        os.path.join(os.path.dirname(model_fn), "np_res_array_1d.npy"), oned_res_arr
    )
...
for z_index in range(mdm.nodes_z.size):
    new_res[pad_n:-pad_n, pad_e:-pad_e, z_index] = interpolate.griddata(
        data_points.T,
        rs[:, z_index],
        (new_north, new_east),
        method="cubic",
...
for n_index in range(new_res.shape[0]):
    for e_index in range(pad_e, new_res.shape[1] - pad_e, 1):
        try:
            ...
            res_decay = decay(
                new_res[left_index, e_index, :], fill_res, np.arange(left_index)
            )
    ...
    try:
        ...
        res_decay = decay(
            new_res[n_index, left_index, :], fill_res, np.arange(left_index)
        )
...
for ii in range(mdm.nodes_z.size):
    gw = a * ii ** 3 + gw_0
    new_res[:, :, ii] = ndimage.gaussian_filter(new_res[:, :, ii], gw)

new_res = 10 ** new_res
new_res[np.where(mdm.res_model > 1e9)] = 1e12
new_res[np.where(mdm.res_model < 0.31)] = 0.3
