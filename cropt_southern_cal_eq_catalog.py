...
data_type = {'names':('year', 'month', 'day', 'hour', 'minute', 'seconds', 
                      'lat', 'lon', 'depth', 'mag'),
            'formats':(np.int, np.int, np.int, np.int, np.int, np.float, 
...
crop_eq_arr = eq_arr[np.where((eq_arr['lat'] >= lat_min) & 
                              (eq_arr['lat'] <= lat_max))]       
crop_eq_arr = crop_eq_arr[np.where((crop_eq_arr['lon'] >= lon_min) & 
                                   (crop_eq_arr['lon'] <= lon_max))]       
...
np.savetxt(save_fn, crop_eq_arr, 
           fmt=['%.0f', '%.0f', '%.0f', '%.0f', '%.0f', '%.2f', 
                '%.4f', '%.4f', '%.4f', '%.2f'], 
            delimiter=',',
...
arc_arr = np.array([crop_eq_arr['lon'], crop_eq_arr['lat'], 
                    crop_eq_arr['depth'], crop_eq_arr['mag']])
save_fn = '{0}_crop_arc.txt'.format(eq_fn[:-4])
np.savetxt(save_fn, arc_arr.T, 
           fmt='%.4f', 
            delimiter=',', 
            header='lon,lat,depth,mag')
print 'wrote crop file to {0}'.format(save_fn)
