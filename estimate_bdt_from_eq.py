...
gs = GridSpec(1, 5, wspace=.1, left=0.08, top=.87, right=.985, bottom=.135)
ax = fig.add_subplot(gs[0:4])

cm = plt.cm.get_cmap('RdYlBu_r')

bins = np.linspace(-2, 40)
n, bins, patches = ax.hist(eq_df.depth,
                           bins=bins,
                           color=(1, .53, .1),
                           edgecolor=(.5, .3, .1),
...
grad = np.atleast_2d(np.linspace(.5, 1, 256)).T
...
ax.text(550, 17, "Brittle-Ductile Transition", va="top", 
...
ax2 = plt.twiny(ax)
ax2.step(med_res, (m.grid_z[0:-1] / 1000.), color=(.15, .35, .85), lw=3)
ax2.set_xscale("log")
ax2.set_xlim(8, 600)
...
l1, = ax3.plot(mag, bins[:-1], color=(.95, .1, .2))
ax3.fill_betweenx(bins[:-1], mag_min, mag_max, color=(.5, .25, .25))

ax3.plot([-2, 5], [15, 15], color="w", lw=1, ls="-.")
ax3.plot([-2, 5], [25, 25], color="w", lw=1, ls="--")

ax3.grid(which="major", lw=.5, zorder=0, color=(.5, .5, .5))
