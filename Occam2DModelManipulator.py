...
class Occam2DModelManipulator(occam2d.Occam2DModel):
    ...
    def _caluculate_midpoints(self):
        ...
        self.xmid = np.array([self.plotx[ii]+(self.plotx[ii+1]-self.plotx[ii])/2 
                          for ii in range(len(self.plotx[:-1]))])
                              
        self.ymid = np.array([self.ploty[ii]+(self.ploty[ii+1]-self.ploty[ii])/2 
                          for ii in range(len(self.ploty[:-1]))])
                              
        #calculate tolerance for each cell to pick around
        self.xpick_tol = np.array([.9*(self.plotx[ii+1]-self.plotx[ii])/2 
                              for ii in range(len(self.plotx[:-1]))])
        self.ypick_tol = np.array([.9*(self.ploty[ii+1]-self.ploty[ii])/2 
                              for ii in range(len(self.ploty[:-1]))])
    ...
    def _make_resisitivity_range(self, res_min, res_max, res_num):
        ...
        #make a default resistivity range on a log scale
        self.res_range = np.array([13, np.log10(.3)])
        self.res_range = np.append(self.res_range, 
                                   np.array([ii for ii in 
...
#                                           [self.alpha_dict[self.res_value]]*4
    ...
    def _get_x_index(self, x1, x2):
        ...
        if x1 < x2:
            ...
            if len(xchange) == 0:
                ...
                xchange = np.where((self.plotx > x1+(x2-x1)/2-self.mesh_width) & 
                                (self.plotx < x1+(x2-x1)/2+self.mesh_width))[0][0]
        ...
        if x1 > x2:
            xchange = np.where((self.plotx <= x1) & (self.plotx >= x2))[0]
            if len(xchange) == 0:
                xchange = np.where((self.plotx > x2+(x1-x2)/2-self.mesh_width) & 
                                (self.plotx < x2+(x1-x2)/2+self.mesh_width))[0][0]
                print xchange
                return [xchange]
    ...
    def _get_y_index(self, y1, y2):
        ...
        if y1 < y2:
            ...
            if len(ychange) == 0:
                ychange = np.where((self.ploty > (y2+y1)/2*.95) & 
                                (self.ploty < (y2+y1)/2*1.05))[0][0]
                print ychange
                return [ychange]
            ...
            ychange = np.where((self.ploty < y1) & (self.ploty > y2))[0]
            if len(ychange) == 0:
                ychange = np.where((self.ploty > (y1+y2)/2*.95) & 
                                (self.ploty < (y1+y2)/2*1.05))[0][0]
                print ychange
                return [ychange]
    ...
    def rewriteMeshFile(self):
        ...
        mfid.write('\n'.join(['     {:.5g}'.format(10**rr) 
                                     for rr in om.res_range[2:]]))
        mfid.write('\n')
    ...
    def rewrite_iter_fn(self):
        ...
        self.new_model_fn = os.path.join(os.path.dirname(self.iterfn), 
                                         'StartingModel')
