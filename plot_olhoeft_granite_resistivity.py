...
res_g = interpolate.interp1d(rdf.temperature[g_index], rdf.resistivity_g[g_index])
...
#                   color=(0, .75, .75))
line_dry = colorline(
    t[::-1], 10 ** res_dry(t)[::-1], ax, cmap=mt_res, norm=plt.Normalize(-1.00, 0.3)
)
line_wet = colorline(
    t[::-1], 10 ** res_wet(t)[::-1], ax, cmap=mt_res, norm=plt.Normalize(0, 1.9)
)

line_g = colorline(
    tg[::-1], 10 ** res_g(tg)[::-1], ax, cmap=mt_res, norm=plt.Normalize(-1, 0.5)
)
...
ax.grid(which="major", lw=0.75, color=(1, 1, 1), ls=":")
ax.grid(which="minor", lw=0.5, color=(0.85, 0.85, 0.85), ls=":")

ax.axvspan(600, 1000, color=(0.2, 0.2, 0.2))  # partial melt
ax.axvspan(300, 450, color=(0.3, 0.3, 0.3))  # HTR
ax.axvspan(188, 300, color=(0.45, 0.45, 0.45))  # NTR
ax.axhspan(75, 145, xmin=0.54, xmax=0.655, color=(0, 0.65, 0.85))  # NTR felsite
ax.axhspan(55, 105, xmin=0.655, xmax=0.75, color=(0, 0.75, 0.9))  # HTR felsite
ax.axhspan(20, 40, xmin=0.805, xmax=0.95, color=(0.95, 0.9, 0))  # Partial melt
