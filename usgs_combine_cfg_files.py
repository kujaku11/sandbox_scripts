...
def check_std(database, tol=0.005):
    """
    ...
    name_labels = database.columns[column_bool].tolist()
    value_arr = np.array(
        database[database.columns[column_bool]], dtype=np.float
    ).flatten()
...
for station in os.listdir(station_dir):
    ...
    cfg_fn_list = sorted(
        [
            ...
            if "mt" not in fn and "runs" not in fn and fn.endswith(".cfg")
        ]
    )
    ...
    for cfg_fn in cfg_fn_list:
        ...
        if count == 0:
            ...
            cfg_db = cfg_db.append(
                check_db(pd.DataFrame([cfg_dict[cfg_dict.keys()[0]]]))
            )
            count += 1

    cfg_db.to_csv(os.path.join(cfg_dir, "{0}_runs.csv".format(station)), index=False)
...
s_db.to_csv(os.path.join(station_dir, "station_info.csv".format(station)), index=False)
l_db.to_csv(os.path.join(station_dir, "location_info.csv".format(station)), index=False)
