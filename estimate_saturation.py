...
g = rp.Glover()
g.phase_01.from_dict({"sigma": 1.0 / 1e9, "phi": 0.88, "m": 0.05, "label": "rock"})
# g.phase_02.from_dict({'sigma':1./.5, 'phi':.08, 'm':3., 'label':'EDL'})
g.phase_02.from_dict({"sigma": 1.0 / 1, "phi": 0.04, "m": 1.5, "label": "fluids"})

res_min, res_max = (60, 190)
n = int(1e4)
res_dict = {"res": [], "phi_r": [], "phi_f": [], "sigma_f": []}
for phi_r, sigma_f in zip(
    np.random.normal(0.85, 0.15, n), np.random.lognormal(0.025, 0.25, n)
):
    if phi_r > 1:
        continue
