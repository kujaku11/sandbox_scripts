...
inv_periods = [0.5, 2, 8, 16, 64, 128, 256]
...
edi_list = [os.path.join(edi_path, "mb{0:03}.edi".format(ss)) for ss in station_list]

edi_list += [os.path.join(edi_path, "LV{0:02}.edi".format(ss)) for ss in lv_list]


# --> make mesh
ws_mesh = ws.WSMesh(edi_list)
ws_mesh.save_path = save_path
ws_mesh.cell_size_east = 1000.0
ws_mesh.cell_size_north = 1000.0
ws_mesh.n_layers = 35
ws_mesh.pad_z = 3
ws_mesh.make_mesh()
ws_mesh.plot_mesh()
ws_mesh.write_initial_file(
    initial_fn=os.path.join(save_path, "WSInitialMesh"), res_list=[100]
)
...
ws_data_fn = os.path.join(
    save_path, "Inv1_{0}".format(folder), "WS_data_{0}_8_small.dat".format(folder)
)
