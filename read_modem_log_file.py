...
class ModEMLog(object):
    ...
    #                       self.log_df.f.median()+self.log_df.f.std()))
            ...
            for ax, label in zip([ax_a, ax_f, ax_l, ax_m2, ax_rms], self._keys):
                ax.grid(which='major', lw=.5, ls='--', color=(.5, .5, .5))
                ax.grid(which='minor', lw=.25, ls='--', color=(.75, .75, .75))
            ...
            for key in self._keys[::-1]:
                if key == 'rms':
                    ...
                    lw = 2
                    zorder = 3
                l1, = ax1.plot(self.log_df.index, self.log_df[key], lw=lw,
                               zorder=zorder)
                line_list.append(l1)
            ...
            ax1.grid(which='major', ls='--', color=(.5, .5, .5), zorder=0)
            ax1.grid(which='minor', ls='--', color=(.75, .75, .75), zorder=0)
