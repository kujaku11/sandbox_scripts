...
if __name__ == "__main__":
    ...
    for sb_station in station_list:
        station_item = archive.sb_upload_data(
            page_id, os.path.join(archive_dir, sb_station), username, password
        )
    et = datetime.datetime.now()
    t_diff = et - st
    print("---> Archived {0}".format(station))
    print(
        "     Took {0:.0f}:{1:02.2f}".format(
            int(t_diff.total_seconds() // 60), t_diff.total_seconds() % 60
        )
    )
