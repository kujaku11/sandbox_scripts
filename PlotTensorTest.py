...
# ---------Forward Model-------------------------------------------------------
    ...
    if layeredyn == "n":
        edipathb = (
            r"c:\Peacock\PHD\Geothermal\Paralana\ForwardModels\TensorTest\HalfSpace"
        )
        if ptype == "sq":
            ...
            edipathi = (
                r"c:\Peacock\PHD\Geothermal\Paralana\ForwardModels\TensorTest\Faults"
            )
            if ttype == "rt":
                fignum = 3
            elif ttype == "pt":
        ...
        if ptype == "sq":
            ...
            edipathi = (
                r"c:\Peacock\PHD\Geothermal\Paralana\ForwardModels\TensorTest\NEFaults"
            )
            if ttype == "rt":
                fignum = 5
            elif ttype == "pt":
    ...
    if sline == "ns":
        pstationlst = (
            ["par12{0}".format(ii) for ii in range(19, 9, -1)]
        ...
        if ttype == "rt":
            if diffyn == "y":
                ...
                for ii in range(nf):
                    ...
                    if noise != None:
                        sigman = (
                            np.sqrt(abs(pt1.rho[ii, 0, 1] * pt1.rho[ii, 1, 0])) * noise
                        )
                        pt1.rho[ii] = pt1.rho[ii] + sigman * np.random.normal(
                            size=(2, 2)
                        )
                        pt2.rho[ii] = pt2.rho[ii] + sigman * np.random.normal(
                            size=(2, 2)
                        )
            ...
            if eheightd > emax or ewidthd > emax:
                ...
                if diffyn == "y":
                    if sline == "ew":
                        ...
                        ellipd = Ellipse(
                            (latlst[ss] * xscaling, yspacing * (nf - ff)),
                            width=ewidthd,
                            height=eheightd,
                            angle=azimutharr[ff, ss],
                        )
