...
# get edi files
edilst = [
    os.path.join(edipath, edi) for edi in os.listdir(edipath) if edi.find(".edi") > 0
]
...
for ll in range(ns):
    ...
    mlst[ll]["model-sounding"].rewriteedi(znew=zf, zvarnew=np.zeros_like(zf).real)
    ...
    lat, lon = utm2ll.UTMtoLL(
        reference_ellipsoid, mlst[ll]["northing"], mlst[ll]["easting"], zone
    )
