...
rfn = r"c:\Users\jpeacock\OneDrive - DOI\MountainPass\modem_inv\inv_07\mp_z05_c05_126.res"
...
line_dict = {0: {"label": "$Z_{xx}$", "color": (0.25, .5, .75)},
             1: {"label": "$Z_{xy}$", "color": (0.25, .25, .75)},
             2: {"label": "$Z_{yx}$", "color": (0.75, .25, .25)},
             3: {"label": "$Z_{yy}$", "color": (0.75, .5, .25)}}
...
rms_cmap = colors.LinearSegmentedColormap(
...
gs1 = gridspec.GridSpec(2, 2, hspace=.25, wspace=.075)
gs2 = gridspec.GridSpecFromSubplotSpec(4, 1, subplot_spec=gs1[1, 0], hspace=.01, wspace=.01)
gs3 = gridspec.GridSpecFromSubplotSpec(4, 1, subplot_spec=gs1[1, 1], hspace=.01, wspace=.01)
...
for ii in range(2):
    for jj in range(2):
        ax1.scatter(r.rms_array["lon"][:] + (dx * (-1)**(ii)),
        ...
        ax3.plot(range(r.rms_array.shape[0]),
...
ax2.set_ylim(.51, 3.49)
ax2.plot(r.period_list, np.repeat(1.55, r.period_list.size),
         color="k", ls="--", lw=1, zorder=1)
ax2.grid(which="major", color=(.5, .5, .5), lw=.5)
ax2.grid(which="minor", color=(.65, .65, .65), lw=.35, ls="--")
ax2.set_axisbelow(True)
ax2.legend(line_list, [line_dict[ii]["label"] for ii in range(4)], 
           loc="upper right",
           prop=label_dict,
           borderaxespad=.05, 
           ncol=4)
...
cb_ax = fig.add_axes([.93, 0.65, 0.0175, 0.25])
