...
def remove_static_shift_spatial_filter(
    ...
    s, z_ss = mt_obj.Z.no_ss(
        reduce_res_factor_x=static_shift_x, reduce_res_factor_y=static_shift_y
    )
...
sp_edi_list = [
    os.path.join(sp_edi_path, sp_edi)
    for sp_edi in os.listdir(sp_edi_path)
    if sp_edi.find(".edi") > 0
]
