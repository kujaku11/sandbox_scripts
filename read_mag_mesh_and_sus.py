...
mesh_fn = r"c:\Users\jpeacock\Documents\Geothermal\Washington\MB\AMP_EffSusc_lp_NoPad.mesh"
sus_fn = r"c:\Users\jpeacock\Documents\Geothermal\Washington\MB\AMP_EffSusc_lp_NoPad.sus"
mb_east, mb_north = (596550., 5397110.)
...
grid_east = np.array([grid_east[0:ii+1].sum() for ii in range(grid_east.size)])/1000.
grid_north = np.array([grid_north[0:ii+1].sum() for ii in range(grid_north.size)])/1000.
grid_z = np.array([grid_z[0:ii+1].sum() for ii in range(grid_z.size)])/1000.

grid_diff_east =(mb_east-east)/1000.
grid_diff_north = (mb_north-north)/1000.
...
gridToVTK(r"c:\Users\jpeacock\Documents\Geothermal\Washington\MB\mag_suscept",
          grid_north,
          grid_east, 
          grid_z, cellData={'Susceptibility':sus_array})
