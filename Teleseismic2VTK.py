...
mt_center = np.array((336800.0, 4167525.0))  # long valley center
cell_size = (2, 2, 2)

grid_shift = (grid_center - mt_center) / 1000.0
# grid_shift = np.array([0, 0])

vel_arr = np.loadtxt(
    vfn,
    delimiter=" ",
    dtype=[("east", np.float), ("north", np.float), ("z", np.float), ("vel", np.float)],
)
...
east = np.arange(
    vel_arr["east"].min(), vel_arr["east"].max() + 2 * cell_size[0], cell_size[0]
)
north = np.arange(
    vel_arr["north"].min(), vel_arr["north"].max() + 2 * cell_size[1], cell_size[1]
)
z = np.arange(vel_arr["z"].min(), vel_arr["z"].max() + 2 * cell_size[2], cell_size[2])
...
gridToVTK(
    os.path.join(
        r"c:\Users\jpeacock\Documents\LV\lv_3d_models", "dawson_teleseismic_vp_lvc16"
    ),
    north - grid_shift[0],
    east - grid_shift[1],
    z,
...
pointsToVTK(
    os.path.join(
        r"c:\Users\jpeacock\Documents\LV\lv_3d_models", "dawson_teleseismic_loc_lvc16"
    ),
