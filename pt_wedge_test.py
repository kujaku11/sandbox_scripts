...
class PlotPTShapes(mtpl.PlotSettings):
    ...
    def set_axis_labels(self, x_label, y_label):
        ...
        add axis labels
        """
        self.ax.set_xlabel(x_label, fontdict={'size':self.font_size,
                                              'weight':'bold'})
        self.ax.set_ylabel(y_label, fontdict={'size':self.font_size,
                                              'weight':'bold'})
    ...
    def get_axis_limits(self):
        ...
        if self.x_limits is None: 
            ...
            self.x_limits = (bb['MinLon'] - self.xpad, 
                             bb['MaxLon'] + self.xpad)
        if self.y_limits is None: 
            bb = self.mt_collection.get_bounding_box()
            self.y_limits = (bb['MinLat'] - self.ypad, 
                             bb['MaxLat'] + self.ypad)
