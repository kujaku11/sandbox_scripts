...
inv_periods = np.array([0.05, 0.3, 0.7, 3, 7, 16, 64, 128])
...
edi_list_dp = [os.path.join(edi_path_dp, "{0}.edi".format(ss)) for ss in station_list]
...
ws_data = ws.WSData(
    edi_list=edi_list_dp, period_list=inv_periods, station_fn=ws_mesh.station_fn
)
...
ws_data.write_data_file(
    save_path=sv_path, data_basename="WS_data_{0:02}_8_small.dat".format(folder)
)

ws_data_fn = os.path.join(
    save_path, "Inv{0:02}".format(folder), "WS_data_{0:02}_8_small.dat".format(folder)
)
