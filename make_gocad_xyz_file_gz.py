...
model_center_proj = gis_tools.project_point_ll2utm(
    model_center[0], model_center[1], epsg=26742
)
...
for kk, zz in enumerate(z):
    for ii, xx in enumerate(east):
        for jj, yy in enumerate(north):
            line = "{0:.4f} {1:.4f} {2:+.4f} {3:.4f}".format(
                xx, yy, zz, res[jj, ii, kk]
            )
            lines.append(line)

with open(
    r"c:\Users\jpeacock\Documents\ClearLake\modem_inv\mt_model_proj.txt", "w"
) as fid:
    fid.write("\n".join(lines))
...
im = plt.pcolormesh(
    plot_x, plot_y, np.log10(res[:, :, 53]), vmin=0, vmax=np.log10(300), cmap="jet_r"
)
plt.colorbar(im, ax=ax)

proj_d = gis_tools.project_point_ll2utm(
    d.station_locations.lat, d.station_locations.lon, epsg=26742
)
plt.scatter(proj_d.easting, proj_d.northing, marker="v", c="k", s=25)

plt.show()

# write out station locations
d_lines = [
    "# USGS MT STATION LOCATIONS",
    "# Author = J. Peacock",
    "# Date = 2017-11-21",
    "# Location = Geysers, CA",
    "# Coordinate System = NAD27 / California 402 State Plane Zone 2 ",
...
with open(
    r"c:\Users\jpeacock\Documents\ClearLake\modem_inv\mt_stations.txt", "w"
) as fid:
    fid.write("\n".join(d_lines))
