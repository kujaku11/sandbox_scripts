...
def pad_res_model(res_array, pad):
    for zz in range(res_array.shape[2]):
        ...
        res_array[:, 0:pad, zz] = res_array[:, pad, zz].repeat(pad).reshape(
                                                  res_array[:, 0:pad, zz].shape)    
        res_array[:, -pad:, zz] = res_array[:, -pad-1, zz].repeat(pad).reshape(
                                                  res_array[:, -pad:, zz].shape)
...
def fill_outside_grid(res_array, n_pad, avg_range):
    ...
    for zz in range(res_array.shape[2]):
#        avg_res_value = np.mean([res_array[x_index, y_index, zz].mean(),
#                                 res_array[avg_range:-avg_range, 0:avg_range, zz].mean(),
#                                 res_array[avg_range:-avg_range, -avg_range:, zz].mean(),
#                                 res_array[0:avg_range, avg_range:-avg_range, zz].mean(),
#                                 res_array[-avg_range:, avg_range:-avg_range, zz].mean()])
        avg_res_value = np.mean([np.median(res_array[x_index, y_index, zz]),
...
smooth_comb_res = fill_outside_grid(comb_res, 12, 14)
...
md = modem.Data()
md.read_data_file(r"c:\Users\jpeacock\Documents\LV\Inversions\lv_mb_err12_tip10.dat")

fig = plt.figure(3)
plt.clf()
                 
ax_mb = fig.add_subplot(2, 2, 1, aspect='equal')
ax_mb.pcolormesh(grid_east, grid_north, np.log10(mb_res[:, :, 33]), 
                 cmap='jet_r',
                 vmin=-1, 
                 vmax=4)
                 
ax_lv = fig.add_subplot(2, 2, 2, aspect='equal', sharex=ax_mb, sharey=ax_mb)
ax_lv.pcolormesh(grid_east, grid_north, np.log10(lv_res[:, :, 33]), 
                 cmap='jet_r',
                 vmin=-1, 
                 vmax=4)
ax_co = fig.add_subplot(2, 2, 3, aspect='equal', sharex=ax_mb, sharey=ax_mb)
ax_co.pcolormesh(grid_east, grid_north, np.log10(comb_res[:, :, 33]), 
                 cmap='jet_r',
                 vmin=-1, 
                 vmax=4)
                 
ax_sm = fig.add_subplot(2, 2, 4, aspect='equal', sharex=ax_mb, sharey=ax_mb)
ax_sm.pcolormesh(grid_east, grid_north, np.log10(smooth_comb_res[:, :, 33]), 
                 cmap='jet_r',
                 vmin=-1, 
                 vmax=4)
                 
for ax in [ax_mb, ax_lv, ax_co, ax_sm]:
    ax.scatter(md.data_array['rel_east'], md.data_array['rel_north'], 
               marker='v', c='k', s=10)
plt.show()
