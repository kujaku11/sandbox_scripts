...
lines.append(
    "# Model Center(lat, lon) WGS84: {0:>+.6f}, {1:>+.6f}".format(
        model_center[0], model_center[1]
    )
)
lines.append(
    "#{0:>12}{1:>12}{2:>12}{3:>12}".format(
        "Northing (m)", "Easting (m)", "Depth (m)", "Log10(res (Ohm-m))"
    )
)
...
save_fn = os.path.join(
    os.path.dirname(mfn), "{0}_resistivity_rel_coord.xyz".format(save_root)
)
...
# --> write data xyz file
d_lines = [
    "{0:<8}{1:>14}{2:>14}{3:>14}{4:>14}{5:>14}".format(
        "station", "rel_east", "rel_north", "elevation", "lat", "lon"
    )
]
for s_arr in d_obj.station_locations:
    d_lines.append(
        "{0:<8}{1:>14.2f}{2:>14.2f}{3:>14.2f}{4:>14.2f}{5:>14.2f}".format(
            s_arr["station"],
            s_arr["rel_east"],
            s_arr["rel_north"],
            s_arr["elev"],
...
save_fn = os.path.join(
    os.path.dirname(mfn), "{0}_stations_rel_coord.xyz".format(save_root)
)
with open(save_fn, "w") as fid:
    fid.write("\n".join(d_lines))
print "Wrote file {0}".format(save_fn)
