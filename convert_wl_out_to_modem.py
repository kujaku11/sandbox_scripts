...
with open(fn, "r") as fid:
    ...
    while n_line[0].lower() != "winglink":
        ...
        if count == 0:
            index_01 = m
        res[index_00:index_01, y_index, z_index] = np.array(
            [float(nn) for nn in n_line]
        )
        count += 1
        index_00 += m
        index_01 += m
    ...
    north, east = [float(value) * 1000.0 for value in loc_lines[2].strip().split()[0:2]]
...
model_obj.grid_center = (
    model_obj.nodes_north.sum() / 2.0,
    model_obj.nodes_east.sum() / 2.0,
    -1 * max_elev,
)
