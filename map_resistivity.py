...
for jj, f in enumerate(range(0, 60, 4)):
    ...
    res_map = spi.griddata(points.T, res_arr, (grid_x, grid_y), method="cubic")

    fig = plt.figure(dpi=150)
    fig.clf()
    fig.subplots_adjust(left=0.07, right=0.95, top=0.99, bottom=0.07)
    ...
    #                  vmax=90)
    im = ax.imshow(
        sps.medfilt2d(np.log10(res_map), kernel_size=(1, 1)),
    ...
    plt.show()
    fig.savefig(
        r"c:\MT\Camas\res_zyx_maps\{0:02}_{1:.4g}_res.png".format(
            jj, 1.0 / mt_obj.Z.freq[f]
        )
    )
    plt.close()
