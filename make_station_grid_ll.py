...
### Shark Tank
ll_utm = {"lon_min": -118.5, "lon_max": -117.9, "lat_min": 38.6, "lat_max": 39.10}
dx = 5
spacing_east = dx / 111.34
spacing_north = dx / 111.34

lat_arr = np.arange(ll_utm["lat_min"], ll_utm["lat_max"] + spacing_north, spacing_north)

lon_arr = np.arange(ll_utm["lon_min"], ll_utm["lon_max"] + spacing_east, spacing_east)
...
for ii, lat_ii in enumerate(lat_arr):
    for jj, lon_jj in enumerate(lon_arr):
        ...
        kml_obj.newpoint(name="GV{0:03}".format(count), coords=[(lon_jj, lat_ii)])
