...
for mt_obj in mt_list:
    period_list.extend(1.0 / mt_obj.Z.freq)
...
period_dict = dict([(np.round(key, 5), value) for value, key in enumerate(period_list)])


for st_index, mt_obj in enumerate(mt_list):
    # make a dictionary that coorelates with index values
    st_period_dict = dict(
        [(np.round(key, 5), value) for value, key in enumerate(1.0 / mt_obj.Z.freq)]
    )
...
ax.grid(True, color=(0.75, 0.75, 0.75), alpha=0.35)

# plot ellipticity
(l2,) = ax.semilogx(period_list, plot_ellip, color=(0.5, 0.5, 0.5), lw=2)
ax.fill_between(period_list, 0, plot_ellip, facecolor=(0.75, 0.75, 0.75))
...
ax.fill_between(period_list, 0, plot_skew, facecolor=(0.5, 0.5, 0.5))
...
for jj, bb in enumerate(plot_period_ranges, 0):
    axpt = fig.add_axes(([0.12 + jj / pr * sax, 0.80, sax / pr, sax / pr]), polar=True)
    ...
    trhist = np.histogram(
        tp_sub[np.nonzero(tp_sub)].flatten(), bins=360 / bw, range=histrange
    )

    # make a bar graph with each bar being width of bw degrees
    bartr = axpt.bar((trhist[1][:-1]) * np.pi / 180, trhist[0], width=bw * np.pi / 180)
    ...
    for cc, bar in enumerate(bartr):
        fc = float(trhist[0][cc]) / trhist[0].max() * 0.75
