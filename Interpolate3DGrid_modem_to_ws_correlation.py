...
ws_model_fn = (
    r"/home/jpeacock/Documents/wsinv3d/LV/sm_modem_inv2/lv_sm_modem_inv2_fine_model.05"
)
...
ws_north, ws_east = np.broadcast_arrays(
    ws_model.grid_north[:, None], ws_model.grid_east[None, :]
)

# make a new resistivity array that will be filled by interpolation
new_res = np.zeros(
    (
        modem_model.grid_north.shape[0],
...
for zz in range(modem_model.grid_z.shape[0]):
    ...
    print "New depth={0:.2f}; old depth={1:.2f}".format(
        modem_model.grid_z[zz], ws_model.grid_z[old_zz]
    )

    # interpolate ws onto modem grid in 2D for each layer
    # be sure to shift the cell
    new_res[:, :, zz] = spi.griddata(
        (ws_north.ravel() - shift_north, ws_east.ravel() + shift_east),
        ws_model.res_model[:, :, old_zz].ravel(),
        (modem_model.grid_north[:, None], modem_model.grid_east[None, :]),
        method="linear",
    )
...
modem_model.write_model_file(
    save_path=r"/home/jpeacock/Documents/ModEM/LV/sm_avg_inv1",
...
for ii, zz in enumerate(range(0, 40, 5), 1):
    ...
    cp = ax.pcolormesh(x, y, np.log10(res_avg[:, :, zz]), cmap="jet_r", vmin=-1, vmax=4)
