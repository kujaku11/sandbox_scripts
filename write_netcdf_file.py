...
model_center_ne = gis_tools.project_point_ll2utm(model_center[0], 
                                                 model_center[1])
...
for ii, north in enumerate(model_center_ne[0]-m_obj.grid_north[:-1]):
    lat_00, lon_00 = gis_tools.project_point_utm2ll(north, 
                                                    model_center_ne[1],
                                                    model_center_ne[2])
    lat[ii] = lat_00
    
for ii, east in enumerate(model_center_ne[1]-m_obj.grid_east[:-1]):
    lat_00, lon_00 = gis_tools.project_point_utm2ll(model_center_ne[0],
                                                    east,
                                                    model_center_ne[2])
    lon[ii] = lon_00
