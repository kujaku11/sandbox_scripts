...
def get_rms_by_period_df(res_obj, rms_thresh=5.0):
    ...
    for ii, key in enumerate(keys):
        z_rms = np.nanmean(res_obj.data_array[ii]['z'].__abs__()/res_obj.data_array[ii]['z_err'].real,
                           axis=0)
        t_rms = np.nanmean(res_obj.data_array[ii]['tip'].__abs__()/res_obj.data_array[ii]['tip_err'].real,
                           axis=0)
        
        
        rms_list = [z_rms.mean(),t_rms.mean(), z_rms[0, 0], z_rms[0, 1],
                    z_rms[1, 0], z_rms[1, 1], t_rms[0, 0], t_rms[0, 1]]
...
def plot_rms_summary(res_obj, ylim=(0, 5), bar_width=0.5,
                     keys=['z', 't', 'zxx', 'zxy', 'zyx', 'zyy', 'tx', 'ty']):
    """
    plot summary as bar plots 
    """
    summary_df = get_rms_summary_df(res_obj)
    ax_list = summary_df[keys].plot.bar(width=bar_width,
                                        subplots=True,
                                        ylim=ylim,
                                        grid=True)
    for ax in ax_list:
...
def plot_rms_by_period(res_obj, ylim=(0, 5), bar_width=0.5,
    ...
    ax_list = period_df[keys].plot.bar(width=bar_width,
                                        subplots=True,
                                        ylim=ylim,
                                        grid=True)
    for ax in ax_list:
