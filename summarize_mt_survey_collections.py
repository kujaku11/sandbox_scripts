...
tol_dict = {
    4096: {"s_diff": 5 * 60, "min_points": 2 ** 18},
    256: {"s_diff": 3 * 3600, "min_points": 2 ** 19},
    4: {"s_diff": 4 * 3600, "min_points": 2 ** 14},
}
...
if sdfn.exists():
    ...
    sdf = c_obj.summarize_survey(survey_path, calibration_path=calibration_path)
...
for station in sdf.station.unique():
    ...
    for sr in station_df.sampling_rate.unique():
        ...
        for start in sr_df.start.unique():
            ...
            rr_df = sdf[
                (sdf.start >= s1) & (sdf.start <= s2) & (sdf.station != station)
            ]
...
for station in info_df.station.unique():
    ...
    for sr in station_df.sampling_rate.unique():
        sr_df = station_df[
            (station_df.sampling_rate == sr) & (station_df.rr_station == rr_list)
        ]
