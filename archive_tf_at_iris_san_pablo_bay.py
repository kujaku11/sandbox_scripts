...
# path to TF files
edi_path = Path(
    r"c:\Users\jpeacock\OneDrive - DOI\MTData\SanPabloBay\EDI_Files"
)
...
for edi_file in edi_path.glob("*.edi"):
    ...
    mt_obj.survey_metadata.funding_source.organization = (
        "U.S. Geological Survey Seismic Hazards Program"
    )
    mt_obj.survey_metadata.funding_source.comments = (
        "This project was funded by U.S. Geologic Survey Seismic Hazards "
        "Program to understand spalys of the Rogers Creek Fault in the "
        "San Pablo Bay region, northern California"
    )
    ...
    mt_obj.survey_metadata.citation_dataset.authors = (
        "J. R. Peacock, K. M. Denton, D. A. Ponce, J. T. Watt"
    )
    mt_obj.survey_metadata.citation_dataset.title = (
        "Magnetotelluric data in San Pablo Bay, northern California"
    )
    ...
    mt_obj.survey_metadata.citation_journal.authors = (
        "Peacock, J.R., Ponce, D.A., Watt, J.T., Denton, K.M "
    )
    mt_obj.survey_metadata.citation_journal.doi = (
        "https://doi.org/10.5066/P96T27UM"
    )
    ...
    mt_obj.station_metadata.geographic_name = "San Pablo Bay, CA, USA"
    ...
    xml_obj.copyright.selected_publications = (
        ...
        f"doi:{mt_obj.survey_metadata.citation_journal.doi}."
    )
