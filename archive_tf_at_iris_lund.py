...
survey_summary = pd.read_csv(
    r"c:\Users\jpeacock\OneDrive - DOI\MTData\LD2024\survey_summary.csv"
)
survey_summary.station = [f"ld{ss}" for ss in survey_summary.station]
...
for edi_file in list(edi_path.glob("*.edi"))[0:1]:
    ...
    mt_obj.survey_metadata.citation_dataset.authors = (
        "J. Peacock, B. Morbeck, W. Schermerhorn"
    )
    mt_obj.survey_metadata.citation_dataset.title = "Magnetotelluric data near Lund, UT for the INGENIOUS geothermal project"
    ...
    mt_obj.station_metadata.geographic_name = "Lund, UT, USA"
    ...
    xml_obj.copyright.additional_info = (
        ...
        "geothermal resources in the region of Lund, UT."
    )
    ...
    xml_obj.site.data_quality_notes.comments.value = "Geomagnetic Storm"
