...
kml_file = (
    r"c:\Users\jpeacock\Documents\ClearLake\Geysers_proposed_MT_just_sites_2017_num.kml"
)
...
lines = [
    ",".join(
        ["{0}".format("station"), "{0}".format("latitude"), "{0}\n".format("longitude")]
    )
]
...
for station, lat, lon in zip(station_list, lat_list, lon_list):
    lines.append(
        ",".join(
            ["{0}".format(station), "{0:.6f}".format(lat), "{0:.6f}\n".format(lon)]
        )
    )
