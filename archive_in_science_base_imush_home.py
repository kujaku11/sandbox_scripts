    ...
    if os.path.isdir(station_path):
        ...
        with Capturing() as output:
            for fn_block in fn_list:
                ...
                if "ZEN18" in [
                    zm.channel_dict[chn]["InstrumentID"]
                    for chn in zm.channel_dict.keys()
                ]:
                    zm.ts.hx *= -1
                ...
                if write_asc:
                    ...
                    # get file name
                    asc_fn = zm._make_file_name(
                        save_path=station_save_dir, compression=False
                    )
                    ...
                    with zipfile.ZipFile(asc_fn + ".zip", "w") as zip_fid:
                        zip_fid.write(
                            asc_fn, os.path.basename(asc_fn), zipfile.ZIP_DEFLATED
                        )
                        os.remove(asc_fn)
        ...
        print(
            "--> Archived station {0}, took {1}:{2}, finished at {3}".format(
                station,
