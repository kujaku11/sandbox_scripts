...
def make_fn_df(df_fn):
    original = Path(r"c:\Users\jpeacock\OneDrive - DOI\Geysers\EDI_Files_birrp\Edited\Geographic")
    phase_01 = Path(r"c:\Users\jpeacock\OneDrive - DOI\Geysers\CEC\EDI_files_birrp_processed\Geographic\Edited")
    ...
    for f1 in original.glob("*.edi"):
        ...
        for f2 in phase_01.glob("*.edi"):
            m2 = MT(f2)
            if np.isclose(m1.latitude, m2.latitude, .0001) and np.isclose(m1.longitude, m2.longitude, .0001):
...
def remove_static_shift(df_fn, nf=22, save_dir=Path(r"c:\Users\jpeacock\OneDrive - DOI\Geysers\CEC\EDI_files_birrp_processed\Geographic\Edited\SS")):
    ...
    for row in df.itertuples():
        ...
        m2.Z = m2.remove_static_shift(1./sx, 1./sy)
