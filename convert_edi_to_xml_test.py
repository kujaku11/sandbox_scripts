...
estimates = [
    Dummy(
        **{
            "_name": "Estimate(type=real)(name=VAR)",
    ...
    Dummy(
        **{
            "_name": "Estimate(type=complex)(name=COV)",
    ...
    Dummy(
        **{
            "_name": "Estimate(type=complex)(name=INVSIGCOV)",
    ...
    Dummy(
        **{
            "_name": "Estimate(type=complex)(name=RESIDCOV)",
    ...
    Dummy(
        **{
            "_name": "Estimate(type=complex)(name=COH)",
    ...
    Dummy(
        **{
            "_name": "Estimate(type=complex)(name=PREDCOH)",
    ...
    Dummy(
        **{
            "_name": "Estimate(type=complex)(name=SIGAMP)",
    ...
    Dummy(
        **{
            "_name": "Estimate(type=complex)(name=SIGNOISE)",
...
data_types = [
    Dummy(
        **{
            "_name": "DataType(units=[mV/km]/[nT])(type=complex)(name=Z)(input=H)(output=E)",
    ...
    Dummy(
        **{
            "_name": "DataType(units=[])(type=complex)(name=T)(input=H)(output=H)",
...
class XML_Config(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.Image = Dummy(**{"PrimaryData": None, "Filename": None, "_name": "Image"})

        self.Original = Dummy(
            **{"Attachment": None, "Filename": None, "_name": "Original"}
        )

        self.TimeSeriesArchived = Dummy(
            **{"Value": 0, "URL": None, "_name": "TimeSeriesArchived"}
        )
        self.ExternalURL = Dummy(
            **{"Description": None, "URL": None, "_name": "ExternalURL"}
        )
        self.PrimaryData = Dummy(
            **{"Filename": None, "GroupKey": 0, "OrderKey": 0, "_name": "PrimaryData"}
        )

        self.Attachment = Dummy(
            **{
                "Filename": None,
        ...
        self.Provenance = Dummy(
            **{
                "CreatTime": datetime.datetime.strftime(
                    datetime.datetime.utcnow(), dt_fmt
                ),
        ...
        self.ProcessingInfo = Dummy(
            **{
                "ProcessedBy": None,
                "ProcessingSoftware": Dummy(
                    **{
                        "Name": None,
                        "LastMod": None,
                        "Author": None,
        ...
        self.Electrode = Dummy(
            **{"Type": None, "Manufacturer": None, "Id": None, "_name": "Electrode"}
        )

        self.Magnetometer = Dummy(
            **{"Type": None, "Manufacturer": None, "Id": None, "_name": "Magnetometer"}
        )

        self.DataQualityNotes = Dummy(
            **{
                "Rating": None,
                "GoodFromPeriod": None,
                "GoodToPeriod": None,
        ...
        self.DataQualityWarnings = Dummy(
            **{"Flag": 0, "Comments": None, "_name": "DataQualityWarnings"}
        )
        ...
        for line in lines:
            ...
            # else assume the line is metadata separated by =
                ...
                if key.find(".") > 0:
                    key_list = key.split(".")
                    if len(key_list) == 2:
                        setattr(getattr(self, key_list[0]), key_list[1], value)
                    elif len(key_list) == 3:
                        setattr(
                            getattr(getattr(self, key_list[0]), key_list[1]),
                            key_list[2],
                            value,
                        )
    ...
    def write_cfg_file(self, cfg_fn=None):
        ...
        for attr_00 in sorted(self.__dict__.keys()):
            ...
            if type(attr_00_value) in [int, float, str]:
                line_list.append(self._write_cfg_line(attr_00, getattr(self, attr_00)))
                ...
                for attr_01 in sorted(attr_00_value.__dict__.keys()):
                    ...
                    if (
                        type(attr_01_value) in [float, int, str]
                        or attr_01_value is None
                    ):
                        line_list.append(
                            self._write_cfg_line(
                                "{0}.{1}".format(attr_00, attr_01), attr_01_value
                            )
                        )
                        ...
                        for attr_02 in sorted(attr_01_value.__dict__.keys()):
                            ...
                            if (
                                type(attr_02_value) in [float, int, str]
                                or attr_02_value is None
                            ):
                                line_list.append(
                                    self._write_cfg_line(
                                        "{0}.{1}.{2}".format(attr_00, attr_01, attr_02),
                                        attr_02_value,
                                    )
                                )
                                ...
                                for attr_03 in sorted(attr_02_value.__dict__.keys()):
                                    ...
                                    if (
                                        type(attr_03_value) in [float, int, str]
                                        or attr_03_value is None
                                    ):
                                        line_list.append(
                                            self._write_cfg_line(
                                                "{0}.{1}.{2}.{3}".format(
                                        ...
                                        for attr_04 in sorted(
                                            attr_03_value.__dict__.keys()
                                        ):
                                            print attr_04
                                            attr_04_value = getattr(
                                                attr_03_value, attr_03
                                            )
                                            if (
                                                type(attr_03_value) in [float, int, str]
                                                or attr_04_value is None
                                            ):
                                                line_list.append(
                                                    self._write_cfg_line(
                                                        "{0}.{1}.{2}.{3}.{4}".format(
...
class EDI_to_XML(object):
    ...
    def get_info(self, cfg_fn=None, edi_fn=None):
        ...
        self.cfg_obj.Site.Location = Dummy(
            **{
                "Latitude": "{0:.6f}".format(self.mt_obj.lat),
        ...
        self.cfg_obj.ProcessingInfo.RemoteInfo.YearCollected = (
            self.cfg_obj.Site.DateCollected
        )
        ...
        self.cfg_obj.FieldNotes.Magnetometer.HX = str(
            self.mt_obj.edi_object.Define_measurement.meas_hx.acqchan
        )
        self.cfg_obj.FieldNotes.Magnetometer.HY = str(
            self.mt_obj.edi_object.Define_measurement.meas_hy.acqchan
        )
        try:
            self.cfg_obj.FieldNotes.Magnetometer.HZ = str(
                self.mt_obj.edi_object.Define_measurement.meas_hz.acqchan
            )
        ...
        self.cfg_obj.PeriodRange._name = "PeriodRange(min={0:.9f})(max={1:.9f})".format(
            (1.0 / self.mt_obj.Z.freq.max()), (1.0 / self.mt_obj.Z.freq.min())
        )
        self.format_data()
    ...
    def format_data(self):
        ...
        comp_dict_t = {(0, 0): ("Tx", "Hx", "Hz"), (0, 1): ("Ty", "Hy", "Hz")}
        ...
        for f_index in range(nf):
            ...
            setattr(
                ...
                Dummy(
                    **{
                        "_name": "Period(units=sec)(value={0:.6g})".format(
                            1.0 / self.mt_obj.Z.freq[f_index]
                        )
                    }
                ),
            )
            ...
            for estimate in ["z", "z.var", "t", "t.var"]:
                ...
                if "z" in estimate:
                    for e_index in range(2):
                        for h_index in range(2):
                            ...
                            key_name = "value(name={0})(input={1})(output={2})".format(
                                c[0], c[1], c[2]
                            )
                            if estimate == "z":
                                z_value = getattr(self.mt_obj.Z, attr_dict[estimate])[
                                    f_index, e_index, h_index
                                ]
                                key_value = "{0:<+.8e} {1:<+.8e}".format(
                ...
                if "t" in estimate and self.mt_obj.Tipper.tipper is not None:
                    for e_index in range(1):
                        for h_index in range(2):
                            ...
                            if estimate == "t":
                                z_value = getattr(
                                    self.mt_obj.Tipper, attr_dict[estimate]
                                )[f_index, e_index, h_index]
                                key_value = "{0:<+.8e} {1:<+.8e}".format(
                                    z_value.real, z_value.imag
                                )
                ...
                # components of Z
                setattr(
                    getattr(self.cfg_obj.Data, "Period_{0:02}".format(f_index)),
                    estimate.capitalize(),
                    Dummy(**value_dict),
                )
        ...
        for element in self._order_list:
                ...
                for attr in sorted(value.__dict__.keys()):
                    if "_name" in attr:
                        ...
                        if isinstance(attr_value, Dummy):
                            # make a new tree limb
                            new_element_02 = self.make_element(
                                new_element, attr_value._name
                            )
                            ...
                            for attr_02 in sorted(attr_value.__dict__.keys()):
                                if "_name" in attr_02:
                                    ...
                                    if isinstance(attr_02_value, Dummy):
                                        new_element_03 = self.make_element(
                                            new_element_02, attr_02_value._name
                                        )
                                        # loop over attributes within the Dummy class, skipping _name
                                        for attr_03 in sorted(
                                            attr_02_value.__dict__.keys()
                                        ):
                                            if "_name" in attr_03:
                                                pass
                                            else:
                                                attr_03_value = getattr(
                                                    attr_02_value, attr_03
                                                )
                                                if isinstance(attr_03_value, Dummy):
                                                    new_element_04 = self.make_element(
                                                        new_element_03,
                                                        attr_03_value._name,
                                                    )
                                                    # loop over attributes within the Dummy class, skipping _name
                                                    for attr_04 in sorted(
                                                        ...
                                                        if "_name" in attr_04:
                                                            ...
                                                            attr_04_value = getattr(
                                                                attr_03_value, attr_04
                                                            )
                                                            if isinstance(
                                                                attr_04_value, Dummy
                                                            ):
                                                                new_element_05 = self.make_element(
                                                                    new_element_04,
                                                                    attr_04_value._name,
                                                                )
                                                                for attr_05 in sorted(
                                                                    attr_04_value.__dict__.keys()
                                                                ):
                                                                    if (
                                                                        "_name"
                                                                        in attr_05
                                        ...
                                        self.write_element(
                                            new_element_02, {attr_02: attr_02_value}
                                        )

                        elif (
                            type(attr_value) in [float, int, str] or attr_value is None
                        ):
            ...
            # write out estimates
                ...
                for element_ii in value:
                    newer_element = self.make_element(new_element, element_ii._name)
