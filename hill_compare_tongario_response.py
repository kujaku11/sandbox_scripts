...
edi_base_list = [os.path.join(base_path, edi) for edi in os.listdir(base_path)
                 if edi.endswith('.edi')]

edi_post_list = [os.path.join(post_path, edi) for edi in os.listdir(post_path)
                 if edi.endswith('.edi')]
...
base_arr = np.zeros(ns, dtype=[('station', '|S10'),
                               ('lat', np.float),
                               ('lon', np.float),
                               ('elev', np.float),
                               ('rel_east', np.float), 
...
post_arr = np.zeros(ns, dtype=[('station', '|S10'),
                               ('lat', np.float),
                               ('lon', np.float),
                               ('elev', np.float),
                               ('rel_east', np.float), 
...
#                    test_arr[:, jj] += b_arr[comp][ii]-p_arr[comp][ii]
                    test_arr[max([kk-pad, 0]):min([kk+pad, nr]), max([jj-pad, 0]):min([jj+pad, nr])] += b_arr[comp][ii]-p_arr[comp][ii]
                    continue
...
#                        vmax=limits[1],
#                        cmap='jet')
        im = ax.pcolormesh((x-x.mean())/1000., (y-y.mean())/1000., 
...
#                       marker='v', s=20, c='k')
            ax.scatter((b_arr['east']-x.mean())/1000., 
                       (b_arr['north']-y.mean())/1000., 
                       marker='v', s=20, c='k')
        if cc == 3 or cc == 4:
...
#        ax.yaxis.set_major_locator(MultipleLocator(1000.))
        ...
        ax.axis('tight')
    fig.suptitle('Difference for {0:.5g} s'.format(1./ff), 
                 fontdict={'size':10, 'weight':'bold'})
    plt.tight_layout()
    plt.show()
    fig.savefig(r"c:\Users\jpeacock\Documents\ShanesBugs\Tongario_Hill\plots\{0:02}_TNG_diff.png".format(ii),
                dpi=300)
