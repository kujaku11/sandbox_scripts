...
class Data_array(object):
    ...
    def get_edi_list(self):
        ...
        if self.edi_path is not None:
            self.edi_list = [os.path.join(self.edi_path, edi) 
    ...
    def get_freq(self):
        ...
        if nf_list.mean() != frequencies.size:
            interp_frequencies = np.logspace(np.log10(frequencies.min()),
    ...
    def fill_data_array(self):
        ...
        for ii, mt_obj in enumerate(self.mt_obj_list):
            ...
            self.data_array[ii]['phimin'][:] = sps.medfilt(mt_obj.pt.phimin[0], 
                                                           kernel_size=self.ks)
            self.data_array[ii]['phimax'][:] = sps.medfilt(mt_obj.pt.phimax[0], 
                                                           kernel_size=self.ks)
            self.data_array[ii]['azimuth'][:] = sps.medfilt(mt_obj.pt.azimuth[0],
                                                            kernel_size=self.ks)
            self.data_array[ii]['skew'][:] = sps.medfilt(mt_obj.pt.beta[0], 
                                                         kernel_size=self.ks)
            self.data_array[ii]['phimin_err'][:] = sps.medfilt(mt_obj.pt.phimin[1], 
                                                           kernel_size=self.ks)
            self.data_array[ii]['phimax_err'][:] = sps.medfilt(mt_obj.pt.phimax[1], 
                                                           kernel_size=self.ks)
            self.data_array[ii]['azimuth_err'][:] = sps.medfilt(mt_obj.pt.azimuth[1],
                                                            kernel_size=self.ks)
            self.data_array[ii]['skew_err'][:] = sps.medfilt(mt_obj.pt.beta[1], 
                                                         kernel_size=self.ks)
            for kk in range(2):
                for ll in range(2):
                    self.data_array[ii]['z'][:, kk, ll] = sps.medfilt(mt_obj.Z.z[:, kk, ll].real, 
                                                         kernel_size=self.ks)+\
                                                         sps.medfilt(mt_obj.Z.z[:, kk, ll].imag, 
                                                         kernel_size=self.ks)*1j
                    self.data_array[ii]['z_err'][:, kk, ll] = sps.medfilt(mt_obj.Z.zerr[:, kk, ll], 
                                                         kernel_size=self.ks)
            for nn in range(2):
                self.data_array[ii]['tip'][:, 0, nn] = sps.medfilt(mt_obj.Tipper.tipper[:, 0, nn].real, 
                                                     kernel_size=self.ks)+\
                                                     sps.medfilt(mt_obj.Tipper.tipper[:, 0, nn].imag, 
                                                     kernel_size=self.ks)*1j
                self.data_array[ii]['tip_err'][:, 0, nn] = sps.medfilt(mt_obj.Tipper.tippererr[:, 0, nn], 
                            kernel_size=self.ks)
    ...
    def write_pickle_data(self, pickle_fn=None):
        ...
        if pickle_fn is None and self.pickle_fn is None:
            if self.edi_path is not None:
                ...
                self.pickle_fn = os.path.join(os.path.dirname(self.edi_list[0]),
                                                'Data.pkl')
        else:
...
#              loc='lower left')
    ...
    ax.set_xlim((10**(np.floor(np.log10(1./p_arr['freq'].max()))), 
                10**(np.floor(np.log10(1./p_arr['freq'].min())))))
    ax.set_xscale('log')
    
    ax.set_ylabel('Angle (deg)')
    ax.set_ylim(-5, 90)
    #ax.axis('tight')
    ax.grid('both', color=(.75, .75, .75), lw=.75)
    plt.tight_layout()
    
    fig.savefig(r"c:\Users\jpeacock\Documents\ShanesBugs\Tongario_Hill\plots_compare_diff\{0}.png".format(b_arr['station']),
