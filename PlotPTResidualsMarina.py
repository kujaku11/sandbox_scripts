...
if not os.path.isfile(pkfn):
    ...
    for ss, station in enumerate(edilst):
        ...
        for ii in range(nf): 
            phi = np.eye(2)-(np.dot(np.linalg.inv(pt1.pt[ii]),
                            pt2.pt[ii]))            
...
#        cb1.set_label('(|$\Delta_{max}$|+|$\Delta_{min}$|)/2')
    ...
    ####----Make scale ellipse
    rect = Rectangle((lonlst[-1]*.95,-yspacing/2),
                   2,
                   3*yspacing)
    rect.set_facecolor((1,1,1))
    rect.set_edgecolor((1,1,1))
    ...
    ellips = Ellipse((lonlst[-1]*.90, yspacing),width = esize,
