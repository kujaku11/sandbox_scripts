...
class EMOCollection:
    ...
    def sort_profile(self, profile_direction=None):
        ...
        for ii in range(len(self.emo_list)):
            ...
            direction[ii]["doi_abs"] = (
                np.mean(self.emo_list[ii].doi_absolute)
                - self.emo_list[ii].location["elevation"]
            )
    ...
    def interpolate_model(self, dx=10, dz=50, method="linear"):
        ...
        for ii, jj in enumerate(profile_sorted["index"]):
            ...
            f = interpolate.interp1d(
                emo.elevation, emo.resistivity, bounds_error=False, fill_value=np.nan
            )
        ...
        model_interp = interpolate.griddata(
            (xb.ravel(), zb.ravel()),
        ...
        # interpolate doi
        doi_rel = np.append(
            np.append(profile_sorted["doi_rel"][0], profile_sorted["doi_rel"]),
            profile_sorted["doi_rel"][-1],
        )
        doi_rel_interp = interpolate.interp1d(distance, doi_rel, kind=method)
    ...
    def plot(
        ...
        im = ax.pcolormesh(
            xg, yg, np.log10(model.T), cmap=cmap, vmin=res_limits[0], vmax=res_limits[1]
        )

        (l1,) = ax.plot(nx, doi, ls="--", lw=1, color=(0.25, 0.25, 0.25))
        ax.fill_between(
...
def plot_station_loop(emo_dir, save_dir=None, fig_type="png"):
    ...
    for folder in emo_dir.iterdir():
        ...
        fig.savefig(
            save_dir.joinpath(f"{folder.name}.{fig_type}"), dpi=300, bbox_inches="tight"
        )
    ...
    for emo_dir in [Path(smooth_dir), Path(blocky_dir)]:
        for emo_fn in get_emo_files_from_dir(emo_dir):
            ...
            for res_key in [col for col in emo_obj.model.columns if "res" in col]:
                entry[res_key.replace("res", "resistivity")] = emo_obj.model[
                    res_key
                ].iloc[-1]

            for thick_key in [col for col in emo_obj.model.columns if "thic" in col]:
                entry[thick_key.replace("thic", "thickness")] = emo_obj.model[
                    thick_key
                ].iloc[-1]
...
line_dict = {
    "00": {"flist": ["T00", "T01", "T02", "T03", "T04", "T05"], "d": "ns"},
...
tem_dir = Path(r"c:\Users\jpeacock\OneDrive - DOI\Geothermal\Umatilla\TEM\Models")
...
for key, fdict in line_dict.items():
    for mtype in ["smooth", "blocky"]:
        ...
        emo_fn_list = get_emo_files_from_dir(model_dir, stations=fdict["flist"])

        emo_collection = EMOCollection(emo_fn_list)
        emo_collection.profile_direction = fdict["d"]

        ax1, fig1 = emo_collection.plot(
            dx=5,
            dz=60,
            method="linear",
            res_limits=(1, 3.0),
        ...
        fig1.savefig(
            tem_dir.joinpath("Figures", f"um_tem_line_{key}_{mtype}.pdf"),
            dpi=300,
            bbox_inches="tight",
        )

        fig1.savefig(
            tem_dir.joinpath("Figures", f"um_tem_line_{key}_{mtype}.png"),
            dpi=300,
            bbox_inches="tight",
        )
