...
class EMTFStats(object):
    ...
    def compute_statistics(self, tf_dir=None):
        ...
        for kk, edi in enumerate(edi_list):
            ...
            for ii in range(2):
                for jj in range(2):
                    ...
                    ### and not 0
                    nz_index = np.where((res > 10e-5) & (res < 10e9) & (res != 0.0))
                    ...
                    stat_array[kk]["res_{0}_diff".format(comp)] = np.std(
                        np.diff(res)
                    ) / abs(np.mean(np.diff(res)))
                    stat_array[kk]["phase_{0}_diff".format(comp)] = np.std(
                        np.diff(phase)
                    ) / abs(np.mean(np.diff(phase)))
                    ### compute tipper
                    if ii == 0:
                        ...
                        if t_index[0].size == 0:
                            ...
                            tmag_err = mt_obj.Tipper.amplitude_err[t_index, 0, jj][0][
                                ::-1
                            ]
        ...
        df.to_csv(os.path.join(edi_dir, "data_quality_statistics.csv"), index=True)
    ...
    def summarize_data_quality(self, quality_df=None, quality_fn=None):
        ...
        summarized_df.to_csv(
            os.path.join(self.tf_dir, "data_quality.csv"), header=False
        )
