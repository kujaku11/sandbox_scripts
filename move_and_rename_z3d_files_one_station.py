...
station = "I013"
station_dir = r"/media/jpeacock/My Passport/iMUSH/OSU_2015/do_not_archive/I013_two_occupations_not_currently_used"
    ...
    for fn in files:
        if fn.lower().endswith(".z3d"):
            ...
            if os.stat(fn_path).st_size < 300000L:
                print(
                    "---> Skipping {0} too small {1}".format(
                        fn, os.stat(fn_path).st_size
                    )
                )
                continue
            if fn.count("_") == 4:
